...
table_path = Path(
    "/Users/hengrui/Desktop/research/gr/athenak/analysis/dynbbh_zoom_sink/tilted_spin_sep4_11orbit.dat"
)
...
def kerr_radius(dx, dy, dz, avec):
    ...
    term = rr - a2
    return np.sqrt(
        0.5 * (term + np.sqrt(term * term + 4.0 * a2 * ahat_dot_x * ahat_dot_x))
    )
...
def masks_and_sigma(data):
    ...
    mask_diagnostic = (
        np.sqrt((x - x1) ** 2 + (y - y1) ** 2 + (z - z1) ** 2) <= diagnostic_radius
    ) | (np.sqrt((x - x2) ** 2 + (y - y2) ** 2 + (z - z2) ** 2) <= diagnostic_radius)
    rho = data.variables["dens"]
    b2 = (
        data.variables["bcc1"] ** 2
        + data.variables["bcc2"] ** 2
        + data.variables["bcc3"] ** 2
    )
...
def collect():
    ...
    for run, label in zip(runs, labels):
        ...
        for path in sorted((run / "bin").glob("torus.mhd_w_bcc.*.bin")):
            ...
            for region, mask in [
                ("horizon", mask_h),
                ("diagnostic_radius", mask_diagnostic),
            ]:
                vals = sigma[mask]
                sigma_rows.append(
                    [
    ...
    return (
        np.array(div_rows, dtype=object),
        np.array(sigma_rows, dtype=object),
        finite_fail,
    )
...
np.savetxt(
    case_dir / "zoom_sink_divb_history.csv",
    div_rows,
    fmt="%s",
    delimiter=",",
...
np.savetxt(
    case_dir / "zoom_sink_sigma_history.csv",
    sigma_rows,
    fmt="%s",
    delimiter=",",
...
for label in labels:
    rows = sigma_rows[
        (sigma_rows[:, 0] == label) & (sigma_rows[:, 1] == "diagnostic_radius")
    ]
    if rows.size:
...
if mhd_files:
    ...
    b2 = (
        last.variables["bcc1"][k0] ** 2
        + last.variables["bcc2"][k0] ** 2
        + last.variables["bcc3"][k0] ** 2
    )
    sigma = b2 / np.maximum(rho, 1.0e-300)
    np.savez(
        case_dir / "zoom_sink_xy_slice_final.npz",
        x=last.x1v,
        y=last.x2v,
        rho=rho,
        bcc1=last.variables["bcc1"][k0],
        bcc2=last.variables["bcc2"][k0],
        bcc3=last.variables["bcc3"][k0],
        sigma=sigma,
        time=last.time,
    )
    ...
    for ax, arr, title in zip(axs, [rho, np.sqrt(b2), sigma], ["rho", "|B|", "sigma"]):
        im = ax.imshow(
            np.log10(np.maximum(arr, 1e-300)), origin="lower", extent=extent, cmap="magma"
        )
...
if div_rows.size and float(np.max(div_rows[:, 2].astype(float))) > 1.0e-10:
    raise SystemExit(
        f"divB exceeded tolerance: {np.max(div_rows[:, 2].astype(float)):.3e}"
    )

print(
    "divB max",
    float(np.max(div_rows[:, 2].astype(float))) if div_rows.size else float("nan"),
)
diagnostic_rows = sigma_rows[sigma_rows[:, 1] == "diagnostic_radius"]
if diagnostic_rows.size:
    print(
        "diagnostic sigma first/last mean",
        float(diagnostic_rows[0, 4]),
        float(diagnostic_rows[-1, 4]),
    )
