...
def main(**kwargs):
    ...
    with open(kwargs["filename"], "rb") as f:
        ...
        while f.tell() < file_size:
            ...
            rr2 = np.maximum(
                x[None, None, :] ** 2 + y[None, :, None] ** 2 + z[:, None, None] ** 2, 1.0
            )
            r2 = 0.5 * (
                rr2 - a2 + np.sqrt((rr2 - a2) ** 2 + 4.0 * a2 * z[:, None, None] ** 2)
            )
            r = np.sqrt(r2)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--r_max", type=float, default=np.inf, help="maximum radius to analyze"
    )
    parser.add_argument(
        "--rho_min", type=float, default=0.0, help="minimum density to analyze"
    )
