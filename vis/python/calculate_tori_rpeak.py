...
def main(**kwargs):
    ...
    if kwargs["torus_type"] == "c":
        ...
        def res(r):
            ...
            h_in = c_h(
                kwargs["spin"], kwargs["r_in"], np.pi / 2.0, l_in, c, n, kwargs["r_in"]
            )
            h_out = c_h(
                kwargs["spin"], kwargs["r_out"], np.pi / 2.0, l_out, c, n, kwargs["r_in"]
            )
            return h_out - h_in
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--n", type=float, help="slope of angular momentum scaling", default=0.0
    )
