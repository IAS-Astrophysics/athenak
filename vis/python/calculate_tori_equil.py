...
def main(**kwargs):
    ...
    im = ax1.pcolormesh(
        x_grid, z_grid, np.log10(data["C"]["rho"]), cmap="inferno", vmin=-8, vmax=0
    )
    ...
    im = ax3.pcolormesh(
        x_grid, z_grid, np.log10(data["FM"]["rho"]), cmap="inferno", vmin=-8, vmax=0
    )
    cmap = matplotlib.cm.get_cmap("inferno")
    ...
    im = ax4.pcolormesh(
        x_grid,
        z_grid,
        np.log10(data["FM"]["rho"] * data["FM"]["tt"]),
        cmap="viridis",
        vmin=-10,
        vmax=-2,
    )
    ...
    plt.tight_layout()
    plt.subplots_adjust(
        left=None, bottom=None, right=None, top=None, wspace=0.3, hspace=-0.2
    )
...
def c_l(r, theta, c, n):
    ...
    def res(ll):
        return (ll / c) ** (2.0 / n) + (ll * g_phiphi + ll**2 * g_tphi) / (
            g_tphi + ll * g_tt
        )  # noqa
...
def c_h(r, theta, ll, c, n, r_edge):
    ...
    if n == 1.0:
        ...
        h *= abs(1.0 - c ** (2.0 / n) * ll ** (2.0 - 2.0 / n)) ** (
            n / (2.0 - 2.0 * n)
...
def c_u_t(r, theta, ll):
    ...
    with warnings.catch_warnings():
        ...
        u_t = -(
            (
                (g_tphi**2 - g_tt * g_phiphi)
                / (g_phiphi + 2.0 * ll * g_tphi + ll**2 * g_tt)
            )
            ** 0.5
        )
    return u_t
...
def fm_f(r, theta, r_peak):
    ...
    with warnings.catch_warnings():
        ...
        f = (
            ...
            * np.log(
                aa / (delta * sigma)
                + ((aa / (delta * sigma)) ** 2 + 4.0 * ls**2 / (delta * s**2)) ** 0.5
            )
