...
# Returns dictionary with cell-centered quantities, grid coordinates and
# dimensions, output time and cycle

# Current limitations:
# - only cartesian grids are supported
# - no mesh refinement is supported
# - native byte order assumed, which is big-endian or little-endian depending
#   on the system this script is run on, which might be different from endian
#   of the machine that produced the output
...
def read_binary(filename):
    ...
    for i in range(len(header_lines)):
        line = header_lines[i]
        # new block in input file
        if (len(line.split(b"<")) > 1 and len(line.split(b">")) > 1):
            flag_mesh = False
            flag_meshblock = False
        if (line == b"<mesh>"):
            flag_mesh = True
    ...
    if (Nx1 < 1 or Nx2 < 1 or Nx3 < 1 or nx1 < 1 or nx2 < 1 or nx3 < 1):
        print("Error in read_binary(): unexpected grid dimensions",
