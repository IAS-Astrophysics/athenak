...
def main(**kwargs):
    ...
    derived_dependencies['beta_inv_rel'] = \
            ('eint', 'velx', 'vely', 'velz', 'bcc1', 'bcc2', 'bcc3')
    ...
    derived_dependencies['pmag_prad'] = \
            ('velx', 'vely', 'velz', 'bcc1', 'bcc2', 'bcc3', 'r00_ff')
    ...
    derived_dependencies['cons_em_rel_t'] = \
            ('velx', 'vely', 'velz', 'bcc1', 'bcc2', 'bcc3')
    derived_dependencies['cons_em_rel_x'] = \
            ('velx', 'vely', 'velz', 'bcc1', 'bcc2', 'bcc3')
    derived_dependencies['cons_em_rel_y'] = \
            ('velx', 'vely', 'velz', 'bcc1', 'bcc2', 'bcc3')
    derived_dependencies['cons_em_rel_z'] = \
            ('velx', 'vely', 'velz', 'bcc1', 'bcc2', 'bcc3')
        ...
        if kwargs['variable'][:8] == 'derived:':
            ...
            if variable_name not in variable_names_base:
                raise RuntimeError('Variable "{0}" not found; options are {{{1}}}.'\
                        .format(variable_name, ', '.join(variable_names_base)))
            variable_names = [variable_name]
            variable_ind = 0
            ...
            for ind, name in zip(variable_inds_sorted, variable_names_sorted):
                if ind == -1:
                    ...
                    cell_data = np.array(struct.unpack(block_cell_format, \
                            f.read(variable_data_size))).reshape(block_nz, block_ny, \
                            block_nx)
    ...
    # Calculate derived quantity related to relativistic magnetic pressure
        ...
        bbz = quantities['bcc3']
        bt, bx, by, bz = \
                three_field_to_four_field(bbx, bby, bbz, ut, ux, uy, uz, u_x, u_y, u_z)
        ...
        if kwargs['dimension'] in ('x', 'y') \
            ...
            if kwargs['horizon']:
                horizon = patches.Ellipse((0.0, 0.0), full_width, full_height, \
                        linestyle=horizon_line_style, facecolor='none', \
...
def lower_vector(at, ax, ay, az, g_tt, g_tx, g_ty, g_tz, g_xx, g_xy, g_xz, g_yy, g_yz, \
        g_zz):
