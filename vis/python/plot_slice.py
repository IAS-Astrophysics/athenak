...
def main(**kwargs):
    ...
    with open(kwargs["data_file"], "rb") as f:
        ...
        if location_size not in (4, 8):
            raise RuntimeError(
                "Only 4- and 8-byte integer types supported for " "location data."
            )
        location_format = "f" if location_size == 4 else "d"
        if variable_size not in (4, 8):
            raise RuntimeError(
                "Only 4- and 8-byte integer types supported for cell " "data."
            )
        ...
        if kwargs["variable"][:8] == "derived:":
            ...
            if variable_name not in derived_dependencies:
                raise RuntimeError(
                    'Derived variable "{0}" not valid; options are " \
                        "{{{1}}}.'.format(
                        variable_name, ", ".join(derived_dependencies.keys())
                    )
                )
            variable_names = []
            variable_inds = []
            for dependency in derived_dependencies[variable_name]:
                if dependency not in variable_names_base:
                    raise RuntimeError(
                        'Requirement "{0}" for "{1}" not found.'.format(
            ...
            if variable_name not in variable_names_base:
                raise RuntimeError(
                    'Variable "{0}" not found; options are {{{1}}}.'.format(
                        variable_name, ", ".join(variable_names_base)
                    )
                )
            variable_names = [variable_name]
            variable_ind = 0
        ...
        if kwargs["variable"] in ["derived:" + name for name in names]:
            ...
            if not power_opacity:
                try:
                    ...
                    raise RuntimeError(
                        "Unable to find absorption opacities in input " "file."
                    )
        ...
        if kwargs["variable"] in ["derived:" + name for name in names] or (
            kwargs["variable"] in ["derived:" + name for name in names_alt]
            and power_opacity
        ):
            if input_data["coord"]["general_rel"] == "true":
                try:
                    length_cgs = (
                        float(input_data["units"]["bhmass_msun"]) * gg_msun_cgs / c_cgs**2
                    )
                except:  # noqa: E722
        ...
        names = ("kappa_a", "kappa_t", "alpha_a", "alpha_t", "tau_a", "tau_t")
        if kwargs["variable"] == "derived:T" or (
            kwargs["variable"] in ["derived:" + name for name in names] and power_opacity
        ):
            if input_data["coord"]["general_rel"] == "true":
        ...
        while f.tell() < file_size:
            ...
            if first_time:
                ...
                if kwargs["dimension"] == "x":
                    ...
                    slice_root_blocks = int(input_data["mesh"]["nx1"]) // int(
                        input_data["meshblock"]["nx1"]
                    )
                if kwargs["dimension"] == "y":
                    ...
                    slice_root_blocks = int(input_data["mesh"]["nx2"]) // int(
                        input_data["meshblock"]["nx2"]
                    )
                if kwargs["dimension"] == "z":
                    ...
                    slice_root_blocks = int(input_data["mesh"]["nx3"]) // int(
                        input_data["meshblock"]["nx3"]
                    )
            ...
            if block_level > max_level_calculated:
                for level in range(max_level_calculated + 1, block_level + 1):
                    if kwargs["location"] <= slice_location_min:
                        ...
                        block_ind_for_level.append(
                            mesh_ind - slice_block_n * block_loc_for_level[-1]
                        )
            ...
            if kwargs["dimension"] == "x":
                extents.append(
                    (block_lims[2], block_lims[3], block_lims[4], block_lims[5])
                )
            if kwargs["dimension"] == "y":
                extents.append(
                    (block_lims[0], block_lims[1], block_lims[4], block_lims[5])
                )
            if kwargs["dimension"] == "z":
                extents.append(
                    (block_lims[0], block_lims[1], block_lims[2], block_lims[3])
                )
                    ...
                    if ind == -1:
                        if kwargs["dimension"] == "x":
                            quantities[name].append(
                                np.full((block_nz, block_ny), block_level)
                            )
                        if kwargs["dimension"] == "y":
                            quantities[name].append(
                                np.full((block_nz, block_nx), block_level)
                            )
                        if kwargs["dimension"] == "z":
                            quantities[name].append(
                                np.full((block_ny, block_nx), block_level)
                            )
                    else:
                        f.seek(cell_data_start + ind * variable_data_size, 0)
                        cell_data = np.array(
                            struct.unpack(block_cell_format, f.read(variable_data_size))
    ...
    if kwargs["variable"] in ["derived:" + name for name in names]:
        ...
        if kwargs["variable"] in ["derived:" + name for name in names]:
            ut, ux, uy, uz = norm_to_coord(uut, uux, uuy, uuz, alpha, betax, betay, betaz)
            u_t, u_x, u_y, u_z = lower_vector(
                ut, ux, uy, uz, g_tt, g_tx, g_ty, g_tz, g_xx, g_xy, g_xz, g_yy, g_yz, g_zz
            )
    ...
    if kwargs["variable"] in ["derived:" + name for name in names]:
        ...
        bby = quantities["bcc2"]
        bbz = quantities["bcc3"]
        bt, bx, by, bz = three_field_to_four_field(
            bbx, bby, bbz, ut, ux, uy, uz, u_x, u_y, u_z
        )
        names = ("bt", "bx", "by", "bz")
        if kwargs["variable"] in ["derived:" + name for name in names]:
        ...
        if kwargs["variable"] in ["derived:" + name for name in names]:
            b_t, b_x, b_y, b_z = lower_vector(
                bt, bx, by, bz, g_tt, g_tx, g_ty, g_tz, g_xx, g_xy, g_xz, g_yy, g_yz, g_zz
            )
            ...
            if kwargs["variable"] == "derived:pmag_rel":
                ...
                with warnings.catch_warnings():
                    ...
                    warnings.filterwarnings(
                        "ignore", message=message, category=RuntimeWarning
                    )
    ...
    if kwargs["variable"] in ["derived:" + name for name in names]:
        ...
        with warnings.catch_warnings():
            ...
            if kwargs["variable"] in ["derived:" + name for name in names]:
                ...
                rrtr, rrtth, rrtph = cks_to_sks_vec_con(
                    quantities["r01"], quantities["r02"], quantities["r03"], bh_a, x, y, z
                )
    ...
    if kwargs["horizon"] or kwargs["horizon_mask"]:
        ...
        if (
            ...
            full_height = (
                2.0
                * (
                    (r_hor**2 + bh_a**2 - kwargs["location"] ** 2)
                    / (1.0 + bh_a**2 / r_hor**2)
                )
                ** 0.5
            )
        ...
        if kwargs["dimension"] == "z" and abs(kwargs["location"]) < r_hor:
            radius = (
                (r_hor**2 + bh_a**2) * (1.0 - kwargs["location"] ** 2 / r_hor**2)
            ) ** 0.5
            if kwargs["horizon_mask"]:
    ...
    if kwargs["ergosphere"]:
        r_hor = 1.0 + (1.0 - bh_a**2) ** 0.5
        if kwargs["dimension"] in ("x", "y") and kwargs["location"] ** 2 < 4.0 + bh_a**2:
            w = np.linspace(
                abs(kwargs["location"]), (4.0 + bh_a**2) ** 0.5, ergosphere_num_points
            )
            z = np.empty_like(w)
            for ind, w_val in enumerate(w):
                ...
                def residual_hor(z_val):
                    ...
                    r2 = 0.5 * (
                        rr2
                        - bh_a**2
                        + ((rr2 - bh_a**2) ** 2 + 4.0 * bh_a**2 * z_val**2) ** 0.5
                    )
                    return r2 - r_hor**2
        ...
        if kwargs["dimension"] == "z" and abs(kwargs["location"]) < r_hor:
            ...
            radius = (
                (r_ergo**2 + bh_a**2) * (1.0 - kwargs["location"] ** 2 / r_ergo**2)
            ) ** 0.5
...
def set_derived_dependencies():
    ...
    derived_dependencies["beta_inv_rel"] = (
        "eint",
        "velx",
        "vely",
        "velz",
    ...
    derived_dependencies["sigma_rel"] = (
        "dens",
        "velx",
        "vely",
        "velz",
    ...
    derived_dependencies["pmag_prad"] = (
        "velx",
        "vely",
        "velz",
        "bcc1",
...
def lower_vector(
    at, ax, ay, az, g_tt, g_tx, g_ty, g_tz, g_xx, g_xy, g_xz, g_yy, g_yz, g_zz
):
