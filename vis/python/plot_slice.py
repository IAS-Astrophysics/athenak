...
def main(**kwargs):
  ...
  with open(kwargs['data_file'], 'rb') as f:
    ...
    if kwargs['horizon'] or kwargs['ergosphere']:
      try:
        ...
        raise RuntimeError('Unable to find black hole mass and spin in input file.')
  ...
  for block_num in range(num_blocks_used):
    plt.imshow(quantity[block_num], cmap=kwargs['cmap'], norm=kwargs['norm'], vmin=vmin, \
        vmax=vmax, interpolation='none', origin='lower', extent=extents[block_num])
  ...
  if kwargs['ergosphere']:
    r_hor = bh_m + (bh_m ** 2 - bh_a ** 2) ** 0.5
    if kwargs['dimension'] in ('x', 'y') \
        and kwargs['location'] ** 2 < 4.0 * bh_m ** 2 + bh_a ** 2:
      xy = np.linspace(abs(kwargs['location']), (4.0 * bh_m ** 2 + bh_a ** 2) ** 0.5, \
