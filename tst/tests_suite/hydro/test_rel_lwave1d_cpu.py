...
def test_run(fv, rv):
        ...
        try:
            for res in _res:
                arguments = ['job/basename=hydro_linwave1d',
                            ...
                            'problem/vy=0.3',
                            'problem/vz=-0.05']
                results = testutils.athenak_run("inputs/lwave_relhydro.athinput", arguments)
                assert results, f"Test failed for res={res}, fv={fv}, rv={rv}, wv={wv}./AthenaK run did not complete successfully."
            ...
            if l1_rms_n64 > maxerror:
                pytest.fail(f"{wv} wave error too large for {rv}+{fv} configuration, "
                        f"error: {l1_rms_n64:g} threshold: {maxerror:g}")
            if l1_rms_n64 / l1_rms_n32 > convrate:
                pytest.fail(f"{wv} wave not converging for {rv}+{fv} configuration, "
    ...
    if l1_rms_l != l1_rms_r and rv != 'ppmx':
        ...
        pytest.fail(f"Errors in L/R-going sound waves not equal for {rv}+{fv} configuration, "
