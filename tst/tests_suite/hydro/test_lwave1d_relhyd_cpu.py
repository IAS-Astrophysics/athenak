...
def test_run(iv, fv, rv):
    ...
    """Run tests of GR hydro.  Skip HLLC since not implemented for GR."""
    if fv != 'hllc':
        for wv in _wave:
            try:
                for res in _res:
                    arguments = ['job/basename=hydro_linwave1d',
                                ...
                                'problem/wave_flag=' + wv]
                    results = testutils.athenak_run("inputs/lwave_relhydro.athinput", arguments)
                    assert results, f"GR hydro test failed for iv={iv}, res={res}, fv={fv}, rv={rv}, wv={wv}./AthenaK run did not complete successfully."
