...
def arguments(iv, rv, fv, wv, res,dim=3):
    ...
    return [f'job/basename=hydro_linwave{dim}d_amr',
                            'time/tlim=1.0',
                            'time/integrator=' + iv,
...
def test_run(fv, rv, dim):
    ...
    for wv in _wave:
        try:
            for res in _res:
                results = testutils.athenak_run(input_file,
                                    flags=arguments(iv, rv, fv, wv, res,dim=dim))
            ...
            ri = _recon.index(rv)
            wi = _wave.index(wv)
            data = athena_read.error_dat(f'hydro_linwave{dim}d_amr-errs.dat')
