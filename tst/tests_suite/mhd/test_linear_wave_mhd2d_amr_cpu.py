...
maxerrors={}
maxerrors['rk3'] = np.array([[1e-06, ],
                            [1e-07, ],
                            [1e-07, ],
                            [1e-07, ]])
convrates={}
convrates['rk3'] = np.array([[0.5],
                            [0.4],
                            [0.4],
                            [0.3]])
...
def arguments(iv, rv, fv, wv, res):
    ...
    return ['job/basename=mhd_linwave2d_amr',
                            'time/tlim=1.0',
                            'time/integrator=' + iv,
                            'mesh/nghost=4',
                            'mesh/nx1=' + repr(res),
...
def test_run(iv, fv, rv):
    ...
    for wv in _wave:
        try:
            for res in _res:
                results = testutils.athenak_run(input_file, arguments(iv, rv, fv, wv, res))
            ...
            ri = _recon.index(rv)
            wi = _wave.index(wv)
            data = athena_read.error_dat('mhd_linwave2d_amr-errs.dat')
