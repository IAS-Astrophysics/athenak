...
_CASES += [
    (
        "3D-L{0}".format(level),
        "tests/divb_amr_3d.athinput",
        "DivBAMR3DL{0}".format(level),
        32 * 32 * 32,
        8,
        [
...
def analyze():
    ...
    for label, _, basename, root_ncell, min_rows, _ in _CASES:
        ...
        if len(data["time"]) < min_rows:
            logger.warning(
                "{0} AMR div(B) history is too short: {1} rows, expected {2}".format(
                    label, len(data["time"]), min_rows
                )
            )
            analyze_status = False
        if max_ncell <= root_ncell:
            logger.warning(
                "{0} AMR div(B) test did not refine: max_ncell={1:g}, "
                "root_ncell={2:g}".format(label, max_ncell, root_ncell)
            )
            analyze_status = False
        if max_ndiv > _MAX_NDIV_TOL:
            logger.warning(
                "{0} max normalized div(B) too large: {1:g} threshold {2:g}".format(
                    label, max_ndiv, _MAX_NDIV_TOL
                )
            )
            analyze_status = False
        if l1_ndiv > _L1_NDIV_TOL:
            logger.warning(
                "{0} L1 normalized div(B) too large: {1:g} threshold {2:g}".format(
                    label, l1_ndiv, _L1_NDIV_TOL
                )
            )
            analyze_status = False
        if l2_ndiv > _L2_NDIV_TOL:
            logger.warning(
                "{0} L2 normalized div(B) too large: {1:g} threshold {2:g}".format(
                    label, l2_ndiv, _L2_NDIV_TOL
                )
            )
            analyze_status = False
