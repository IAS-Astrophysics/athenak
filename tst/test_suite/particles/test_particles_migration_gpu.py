"""Deterministic GPU characterization test for particle MeshBlock migration."""

from pathlib import Path

import athena_read
import numpy as np
import pytest

import test_suite.testutils as testutils


HISTORY = Path("particle_migration_gpu.user.hst")


def test_particle_migration_gpu():
    """Drift particles across a MeshBlock boundary and update their owners."""
    try:
        HISTORY.unlink(missing_ok=True)
        assert testutils.run(
            "inputs/particle_migration.athinput"
        ), "particle migration run failed"

        if not HISTORY.exists():
            pytest.fail(f"particle migration history was not written: {HISTORY}")
        data = athena_read.hst(str(HISTORY))

        if not np.all(np.isfinite(data["max_err"])):
            pytest.fail("particle migration error contains non-finite values")
        if data["max_err"][-1] > 1.0e-13:
            pytest.fail(f"particle migration error is too large: {data['max_err'][-1]:g}")
        if data["npart"][-1] != pytest.approx(8.0):
            pytest.fail(f"particle count changed: {data['npart'][-1]:g}")
        if data["tag_sum"][-1] != pytest.approx(28.0):
            pytest.fail(f"particle identifiers changed: tag sum={data['tag_sum'][-1]:g}")
        if data["owner_err"][-1] != pytest.approx(0.0):
            pytest.fail(f"particle owners are incorrect: {data['owner_err'][-1]:g}")
        if data["migrated"][-1] != pytest.approx(2.0):
            pytest.fail(f"wrong migration count: {data['migrated'][-1]:g}")
    finally:
        HISTORY.unlink(missing_ok=True)
