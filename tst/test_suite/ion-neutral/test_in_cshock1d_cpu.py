...
def test_run(iv, res):
    ...
    try:
        results = testutils.athenak_run(input_file,arguments(iv, rv, res))
        assert results, f"cshock test run failed for {iv}+{rv}."
        maxerr = errors[(iv)]
        data = athena_read.error_dat(f'cshock-errs.dat')
        ...
        if l1_rms_err > maxerr:
            pytest.fail(f"Cshock error too large for {iv}+{fv}, "
