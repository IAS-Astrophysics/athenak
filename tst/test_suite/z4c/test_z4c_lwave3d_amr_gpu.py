...
errors={
    ('2nd-order'): (3.5e-11,0.25),
    ('6th-order'): (6.0e-12,0.0)}

_res = [32, 64]     # resolutions to test

def arguments(res, ng):
    """Assemble arguments for run command"""
    return ['mesh/nghost=' + repr(ng),
            'mesh/nx1=' + repr(res),
            'mesh/nx2=' + repr(res),
            'mesh/nx3=' + repr(res),
...
def test_run_2nd():
    ...
    try:
        ...
        if l1_rms_err1 > maxerror:
            pytest.fail(f"2nd-order Z4c wave error too large,"
                        f"error: {l1_rms_err1:g} threshold: {maxerror:g}")
        if (l1_rms_err1 / l1_rms_err0) > errorratio:
            pytest.fail(f"2nd-order Z4c wave converging too slow,"
...
def test_run_6th():
    ...
    try:
        ...
        if l1_rms_err0 > maxerror:
            pytest.fail(f"6th-order Z4c wave error too large,"
                        f"error: {l1_rms_err1:g} threshold: {maxerror:g}")
    finally:
        testutils.cleanup()
