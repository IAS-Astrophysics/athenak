...
\(\partial_t\gamma_{ij}\).  In the present implementation the radiation field
is metric-passive: it is transported on ADM/Z4c geometry, but its
stress-energy is not included in the Z4c metric right-hand side.  We also add a
diagnostic null-geodesic particle pusher for beam-edge tracers on ADM
backgrounds.  Flat-space beam tests
...
The ADM CFL limiter has three parts: coordinate spatial speeds,
angular-geodesic speeds, and the explicit geometric source estimate.  In runs
with radiation excision, all three reductions skip cells marked by
\code{rad\_mask}; otherwise masked cells with singular or intentionally
unphysical metric data can set a meaningless global timestep.
...
current run it passed all 20 checks: eight serial solver inputs, five two-rank
MPI inputs, the write/resume restart pair, the expected fatal rejection of
\code{dyn\_radiation} \code{geometry=cks} with \code{z4c}, the expected dynbbh
rejection of legacy \code{radiation}, two serial dynbbh inputs, and one
two-rank dynbbh smoke test.  The harness scans text outputs and logs for
non-finite values.
...
\code{geometry=adm} and \code{angular\_fluxes=true}.  The dynbbh initializer
...
\(\max R^{00}=0.768586\), wrote a normal-size particle track file, and generated
...
and completed without NaNs.  The two-rank dynbbh smoke test also advanced past
cycle zero and completed to its cycle limit.  These are compact runtime tests;
long production dynbbh/MHD radiation evolutions remain a separate validation
target.
...
time-dependent metric backgrounds.  The explicit ADM update clips negative
intensity to zero for positivity; a counter for that floor activation is still
needed so production runs can monitor any silent radiation-energy loss.
...
device views and POD scalars.  The tests reported here were run on the local
CPU/MPI builds.  No CUDA, HIP, or SYCL runtime validation was available in this
pass, so GPU execution should be treated as statically reviewed but not
runtime-certified.
...
timesteps, restart coverage, and serial plus two-rank ADM beam/null-particle
smoke tests on the superposed binary black-hole background.  The remaining work
is to add radiation metric backreaction, strengthen the momentum-source and
matter-coupling validation suite, monitor positivity clipping, and upgrade the
diagnostic particle pusher if high-accuracy geodesics are required.
