...
TaskStatus Particles::AdjustMeshRefinement(Driver *pdriver, int stage) {
  ...
  KOKKOS_LAMBDA(const int p) {
    if (pi(PLASTMOVE,p) >= 0) {
      ...
      if (level > lastlevel) {
        // higher refinement level: zones are smaller now
        ...
        // get fluxes into the sub-zones the particle could have ended up in
        Real flx1 = 0., flx2 = 0., flx3 = 0., flx4 = 0.;
        ...
        auto rand_gen = rand_pool64_.get_state();
        Real rand = rand_gen.frand();
        rand_pool64_.free_state(rand_gen);
        ...
        // lower refinement level: zones are larger now
        // move the particle to the center of the new zone
