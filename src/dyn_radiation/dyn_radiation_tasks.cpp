...
void DynRadiation::AssembleRadTasks(std::map<std::string, std::shared_ptr<TaskList>> tl) {
  TaskID none(0);
  hydro::Hydro *phyd = pmy_pack->phydro;
  mhd::MHD *pmhd = pmy_pack->pmhd;

  // construct task list depending on enabled physics modules and dyn_radiation parameters
  if (pmhd != nullptr && !(fixed_fluid)) {  // dyn_radiation magnetohydrodynamics
    // assemble "before_stagen" task list
    id.rad_irecv = tl["before_stagen"]->AddTask(&DynRadiation::InitRecv, this, none);
    ...
    id.rad_src   = tl["stagen"]->AddTask(&DynRadiation::RadSrcTerms, this, id.rad_rkupdt);
    ...
    id.rad_coupl = tl["stagen"]->AddTask(&DynRadiation::RadFluidCoupling,this,id.mhd_ct);
    id.rad_resti = tl["stagen"]->AddTask(&DynRadiation::RestrictI, this, id.rad_coupl);
    id.rad_sendi = tl["stagen"]->AddTask(&DynRadiation::SendI, this, id.rad_resti);
    id.rad_recvi = tl["stagen"]->AddTask(&DynRadiation::RecvI, this, id.rad_sendi);
  ...
  } else if (phyd != nullptr && !(fixed_fluid)) {  // dyn_radiation hydrodynamics
    // assemble "before_stagen" task list
    id.rad_irecv = tl["before_stagen"]->AddTask(&DynRadiation::InitRecv, this, none);
    ...
    id.rad_coupl = tl["stagen"]->AddTask(&DynRadiation::RadFluidCoupling,this,id.hyd_src);
    id.rad_resti = tl["stagen"]->AddTask(&DynRadiation::RestrictI, this, id.rad_coupl);
    id.rad_sendi = tl["stagen"]->AddTask(&DynRadiation::SendI, this, id.rad_resti);
    id.rad_recvi = tl["stagen"]->AddTask(&DynRadiation::RecvI, this, id.rad_sendi);
    ...
    id.rad_rkupdt= tl["stagen"]->AddTask(&DynRadiation::RKUpdate, this, id.rad_recvf);
    id.rad_src   = tl["stagen"]->AddTask(&DynRadiation::RadSrcTerms, this, id.rad_rkupdt);
    id.rad_coupl = tl["stagen"]->AddTask(&DynRadiation::RadFluidCoupling,this,id.rad_src);
    id.rad_resti = tl["stagen"]->AddTask(&DynRadiation::RestrictI, this, id.rad_coupl);
    id.rad_sendi = tl["stagen"]->AddTask(&DynRadiation::SendI, this, id.rad_resti);
    id.rad_recvi = tl["stagen"]->AddTask(&DynRadiation::RecvI, this, id.rad_sendi);
...
//----------------------------------------------------------------------------------------
//! \fn TaskStatus DynRadiation::SendFlux
...
TaskStatus DynRadiation::SendFlux(Driver *pdrive, int stage) {
...
//----------------------------------------------------------------------------------------
//! \fn TaskStatus DynRadiation::RecvFlux
//! \brief Wrapper task list function to recv/unpack restricted values of fluxes of
//! conserved variables at fine/coarse boundaries

TaskStatus DynRadiation::RecvFlux(Driver *pdrive, int stage) {
  TaskStatus tstat = TaskStatus::complete;
  // Only execute BoundaryValues function with SMR/SMR
  if (pmy_pack->pmesh->multilevel) {
...
//----------------------------------------------------------------------------------------
//! \fn TaskList DynRadiation::RadSrcTerms
...
//----------------------------------------------------------------------------------------
//! \fn TaskStatus DynRadiation::RestrictI
//! \brief Wrapper task list function to restrict conserved vars

TaskStatus DynRadiation::RestrictI(Driver *pdrive, int stage) {
...
//----------------------------------------------------------------------------------------
//! \fn TaskStatus DynRadiation::ApplyPhysicalBCs
//! \brief Wrapper task list function to call funtions that set physical and user BCs

TaskStatus DynRadiation::ApplyPhysicalBCs(Driver *pdrive, int stage) {
...
//----------------------------------------------------------------------------------------
//! \fn TaskStatus DynRadiation::ClearSend
//! \brief Wrapper task list function that checks all MPI sends have completed.

TaskStatus DynRadiation::ClearSend(Driver *pdrive, int stage) {
...
//----------------------------------------------------------------------------------------
//! \fn TaskStatus DynRadiation::ClearRecv
...
TaskStatus DynRadiation::ClearRecv(Driver *pdrive, int stage) {
