  ...
  if (eos.is_ideal) {
    ...
    par_for_inner(member, il, iu, [&](const int i) {
      ...
      // Compute L/R states for selected conserved variables
      {
        ...
        Real pbl = 0.5*(bxsq + (SQR(wl(_iby,i)) + SQR(wl(_ibz,i))));  // magnetic pressure (l/r)
        Real pbr = 0.5*(bxsq + (SQR(wr(_iby,i)) + SQR(wr(_ibz,i))));
        Real kel = 0.5*wl(IDN,i)*(SQR(wl(ivx,i)) + (SQR(wl(_ivy,i)) + SQR(wl(_ivz,i))));
        Real ker = 0.5*wr(IDN,i)*(SQR(wr(ivx,i)) + (SQR(wr(_ivy,i)) + SQR(wr(_ivz,i))));

        MHDCons1D ul,ur;  // L/R states, conserved variables (computed)
        ul.d  = wl(IDN,i);
        ul.mx = wl(ivx,i)*ul.d;
        ul.my = wl(_ivy,i)*ul.d;
        ul.mz = wl(_ivz,i)*ul.d;
        ...
        ur.d  = wr(IDN,i);
        ur.mx = wr(ivx,i)*ur.d;
        ur.my = wr(_ivy,i)*ur.d;
        ur.mz = wr(_ivz,i)*ur.d;
        ...
        Real cfl = eos.IdealMHDFastSpeed(wl(IDN,i), wl_ipr, bxi, wl(_iby,i), wl(_ibz,i));
        Real cfr = eos.IdealMHDFastSpeed(wr(IDN,i), wr_ipr, bxi, wr(_iby,i), wr(_ibz,i));
      ...
      // }
        ...
        fl.e  = wl(ivx,i)*(ul.e + ptl - bxsq) - bxi*(wl(_ivy,i)*ul.by + wl(_ivz,i)*ul.bz);
        fl.by = ul.by*wl(ivx,i) - bxi*wl(_ivy,i);
        fl.bz = ul.bz*wl(ivx,i) - bxi*wl(_ivz,i);

        fr.d  = ur.mx;
        fr.mx = ur.mx*wr(ivx,i) + ptr - bxsq;
        fr.my = ur.my*wr(ivx,i) - bxi*ur.by;
        fr.mz = ur.mz*wr(ivx,i) - bxi*ur.bz;
        fr.e  = wr(ivx,i)*(ur.e + ptr - bxsq) - bxi*(wr(_ivy,i)*ur.by + wr(_ivz,i)*ur.bz);
        fr.by = ur.by*wr(ivx,i) - bxi*wr(_ivy,i);
        fr.bz = ur.bz*wr(ivx,i) - bxi*wr(_ivz,i);
        ...
        Real ptst = 0.5*(ptl + ul.d*sdl*(spd[2]-wl(ivx,i)) + ptr + ur.d*sdr*(spd[2]-wr(ivx,i)));  // total pressure (star state)

        // ul* - eqn (39) of M&K
        ulst.mx = ulst.d * spd[2];
        if (fabs(ul.d*sdl*sdml-bxsq) < (HLLD_SMALL_NUMBER)*ptst) {
          // Degenerate case
          ulst.my = ulst.d * wl(_ivy,i);
          ulst.mz = ulst.d * wl(_ivz,i);
          ...
          ulst.my = ulst.d * (wl(_ivy,i) - ul.by*tmp);
          ulst.mz = ulst.d * (wl(_ivz,i) - ul.bz*tmp);
        ...
        ulst.e = (sdl*ul.e - ptl*wl(ivx,i) + ptst*spd[2] +
                  bxi*(wl(ivx,i)*bxi + (wl(_ivy,i)*ul.by + wl(_ivz,i)*ul.bz) - vbstl))/sdml;

        // ur* - eqn (39) of M&K
        urst.mx = urst.d * spd[2];
        if (fabs(ur.d*sdr*sdmr - bxsq) < (HLLD_SMALL_NUMBER)*ptst) {
          // Degenerate case
          urst.my = urst.d * wr(_ivy,i);
          urst.mz = urst.d * wr(_ivz,i);
          ...
          urst.my = urst.d * (wr(_ivy,i) - ur.by*tmp);
          urst.mz = urst.d * (wr(_ivz,i) - ur.bz*tmp);
        ...
        urst.e = (sdr*ur.e - ptr*wr(ivx,i) + ptst*spd[2] +
                  bxi*(wr(ivx,i)*bxi + (wr(_ivy,i)*ur.by + wr(_ivz,i)*ur.bz) - vbstr))/sdmr;
  ...
  //------------------------- ISOTHERMAL HLLD solver -------------------------------------
    ...
    par_for_inner(member, il, iu, [&](const int i) {
      ...
      // Compute L/R states for selected conserved variables
      {
        ...
        Real cfl = eos.IdealMHDFastSpeed(wl(IDN,i), bxi, wl(_iby,i), wl(_ibz,i));
        Real cfr = eos.IdealMHDFastSpeed(wr(IDN,i), bxi, wr(_iby,i), wr(_ibz,i));
        ...
        // total pressures L,R
        Real bxsq = bxi*bxi;
        Real ptl = SQR(iso_cs)*wl(IDN,i) + 0.5*(bxsq + SQR(wl(_iby,i)) + SQR(wl(_ibz,i)));
        Real ptr = SQR(iso_cs)*wr(IDN,i) + 0.5*(bxsq + SQR(wr(_iby,i)) + SQR(wr(_ibz,i)));
        ...
        if (fabs(spd[0]-spd[1]) < (HLLD_SMALL_NUMBER)*iso_cs) {
          ...
          ulst.my = dhll*wl(_ivy,i) - ul.by*mfact; // eqn. (30) of Mignone
          ulst.mz = dhll*wl(_ivz,i) - ul.bz*mfact; // eqn. (31) of Mignone
        ...
        if (fabs(spd[4]-spd[3]) < (HLLD_SMALL_NUMBER)*iso_cs) {
          ...
          urst.my = dhll*wr(_ivy,i) - ur.by*mfact; // eqn. (30) of Mignone
          urst.mz = dhll*wr(_ivz,i) - ur.bz*mfact; // eqn. (31) of Mignone
        ...
        if (spd[0] >= 0.0) {
          ...
          flx(m,_ivy,k,j,i) = fl.my + spd[0]*(ulst.my - ul.my);
          flx(m,_ivz,k,j,i) = fl.mz + spd[0]*(ulst.mz - ul.mz);
          ...
          flx(m,_ivy,k,j,i) = fr.my + spd[4]*(urst.my - ur.my);
          flx(m,_ivz,k,j,i) = fr.mz + spd[4]*(urst.mz - ur.mz);
          ...
          flx(m,_ivy,k,j,i) = ucst.my*ustar - bxi*ucst.by;
          flx(m,_ivz,k,j,i) = ucst.mz*ustar - bxi*ucst.bz;
