...
TaskStatus MHD::ApplyPhysicalBCs(Driver* pdrive, int stage)
{
  ...
  for (int m=0; m<nmb; ++m) {
    ...
    switch (pmy_pack->pmb->mb_bcs.h_view(m,BoundaryFace::inner_x1)) {
    ...
    switch (pmy_pack->pmb->mb_bcs.h_view(m,BoundaryFace::outer_x1)) {
      case BoundaryFlag::reflect:
        ReflectOuterX1(m);
        break;
      case BoundaryFlag::outflow:
  ...
  for (int m=0; m<nmb; ++m) {
    ...
    switch (pmy_pack->pmb->mb_bcs.h_view(m,BoundaryFace::inner_x2)) {
      case BoundaryFlag::reflect:
        ReflectInnerX2(m);
        break;
      case BoundaryFlag::outflow:
    ...
    // apply physical bounaries to outer_x1
    switch (pmy_pack->pmb->mb_bcs.h_view(m,BoundaryFace::outer_x2)) {
      case BoundaryFlag::reflect:
        ReflectOuterX2(m);
        break;
      case BoundaryFlag::outflow:
  ...
  for (int m=0; m<nmb; ++m) {
    ...
    switch (pmy_pack->pmb->mb_bcs.h_view(m,BoundaryFace::inner_x3)) {
      case BoundaryFlag::reflect:
        ReflectInnerX3(m);
        break;
      case BoundaryFlag::outflow:
    ...
    switch (pmy_pack->pmb->mb_bcs.h_view(m,BoundaryFace::outer_x3)) {
      case BoundaryFlag::reflect:
        ReflectOuterX3(m);
        break;
      case BoundaryFlag::outflow:
