...
//----------------------------------------------------------------------------------------
//! \fn TaskStatus MHD::AccumulateDensityFlux()
//! \brief Accumulate the RK2 density transfer through each active face over one step.

TaskStatus MHD::AccumulateDensityFlux(Driver *pdrive, int stage) {
  ...
  par_for("mhd_accumulate_density_flux_x1", DevExeSpace(),
          0, nmb1, ks, ke, js, je, is, ie+1,
  ...
  if (pmy_pack->pmesh->multi_d) {
    par_for("mhd_accumulate_density_flux_x2", DevExeSpace(),
            0, nmb1, ks, ke, js, je+1, is, ie,
    KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
  ...
  if (pmy_pack->pmesh->three_d) {
    par_for("mhd_accumulate_density_flux_x3", DevExeSpace(),
            0, nmb1, ks, ke+1, js, je, is, ie,
    KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
