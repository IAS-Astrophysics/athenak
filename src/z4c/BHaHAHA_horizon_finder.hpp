...
private:
  enum class CenterMode { Tracker, MinimumLapse };
  enum class FindScheduleMode { Dcycle, Dt };

  // Initialization
  void LoadParameters();
  bool ShouldFindHorizons() const;
  ...
  CenterMode center_mode_;
  FindScheduleMode find_schedule_mode_;
  ...
  std::array<double, 3> min_lapse_center_;
