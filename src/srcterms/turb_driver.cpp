...
void TurbulenceDriver::Initialize() {
  ...
  auto force_ = force;
  par_for(
      "force_init_pgen", DevExeSpace(), 0, nmb - 1, 0, 2, 0, ncells3 - 1, 0,
      ncells2 - 1, 0, ncells1 - 1,
...
//  \brief Includes task in the operator split task list that constructs new
//  modes with random amplitudes and phases that can be used to evolve the force
//  via an O-U process Called by MeshBlockPack::AddPhysics() function
...
//  \brief includes task in the stage_run task list for adding random forcing to
//  fluid as an explicit source terms in each stage of integrator Called by
//  MeshBlockPack::AddPhysics() function
...
// Cannot be included in constructor since (it seems) Kokkos::par_for not
// allowed in cons.
        ...
        if (nsqr >= nlow_sqr && nsqr <= nhigh_sqr && flag_prl) {
          ...
          if (driving_type == 0) {
            ...
            if (nkz != 0) {
              ...
              xssc_.h_view(nmode) =
                  (nkx == 0 || nky == 0) ? 0.0 : RanGaussianSt(&(rstate));
              xsss_.h_view(nmode) =
                  (nkx == 0 || nky == 0) ? 0.0 : RanGaussianSt(&(rstate));
              ...
              // imcompressibility
              zccc_.h_view(nmode) =
                  ikz * (kx * xscs_.h_view(nmode) + ky * ycss_.h_view(nmode));
              zccs_.h_view(nmode) =
                  -ikz * (kx * xscc_.h_view(nmode) + ky * ycsc_.h_view(nmode));
              zcsc_.h_view(nmode) =
                  ikz * (kx * xsss_.h_view(nmode) - ky * yccs_.h_view(nmode));
              zcss_.h_view(nmode) =
                  ikz * (-kx * xssc_.h_view(nmode) + ky * yccc_.h_view(nmode));
              zscc_.h_view(nmode) =
                  ikz * (-kx * xccs_.h_view(nmode) + ky * ysss_.h_view(nmode));
              zscs_.h_view(nmode) =
                  ikz * (kx * xccc_.h_view(nmode) - ky * yssc_.h_view(nmode));
              zssc_.h_view(nmode) =
                  -ikz * (kx * xcss_.h_view(nmode) + ky * yscs_.h_view(nmode));
              zsss_.h_view(nmode) =
                  ikz * (kx * xcsc_.h_view(nmode) + ky * yscc_.h_view(nmode));
            } else if (nky != 0) {  // kz == 0
  ...
  for (int n = 0; n < mode_count_; n++) {
    par_for(
        ...
        KOKKOS_LAMBDA(int m, int k, int j, int i) {
          force_tmp_(m, 0, k, j, i) += xccc_.d_view(n) * xcos_(m, n, i) *
                                       ycos_(m, n, j) * zcos_(m, n, k);
          force_tmp_(m, 0, k, j, i) += xccs_.d_view(n) * xcos_(m, n, i) *
                                       ycos_(m, n, j) * zsin_(m, n, k);
          force_tmp_(m, 0, k, j, i) += xcsc_.d_view(n) * xcos_(m, n, i) *
                                       ysin_(m, n, j) * zcos_(m, n, k);
          force_tmp_(m, 0, k, j, i) += xcss_.d_view(n) * xcos_(m, n, i) *
                                       ysin_(m, n, j) * zsin_(m, n, k);
          force_tmp_(m, 0, k, j, i) += xscc_.d_view(n) * xsin_(m, n, i) *
                                       ycos_(m, n, j) * zcos_(m, n, k);
          force_tmp_(m, 0, k, j, i) += xscs_.d_view(n) * xsin_(m, n, i) *
                                       ycos_(m, n, j) * zsin_(m, n, k);
          force_tmp_(m, 0, k, j, i) += xssc_.d_view(n) * xsin_(m, n, i) *
                                       ysin_(m, n, j) * zcos_(m, n, k);
          force_tmp_(m, 0, k, j, i) += xsss_.d_view(n) * xsin_(m, n, i) *
                                       ysin_(m, n, j) * zsin_(m, n, k);

          force_tmp_(m, 1, k, j, i) += yccc_.d_view(n) * xcos_(m, n, i) *
                                       ycos_(m, n, j) * zcos_(m, n, k);
          force_tmp_(m, 1, k, j, i) += yccs_.d_view(n) * xcos_(m, n, i) *
                                       ycos_(m, n, j) * zsin_(m, n, k);
          force_tmp_(m, 1, k, j, i) += ycsc_.d_view(n) * xcos_(m, n, i) *
                                       ysin_(m, n, j) * zcos_(m, n, k);
          force_tmp_(m, 1, k, j, i) += ycss_.d_view(n) * xcos_(m, n, i) *
                                       ysin_(m, n, j) * zsin_(m, n, k);
          force_tmp_(m, 1, k, j, i) += yscc_.d_view(n) * xsin_(m, n, i) *
                                       ycos_(m, n, j) * zcos_(m, n, k);
          force_tmp_(m, 1, k, j, i) += yscs_.d_view(n) * xsin_(m, n, i) *
                                       ycos_(m, n, j) * zsin_(m, n, k);
          force_tmp_(m, 1, k, j, i) += yssc_.d_view(n) * xsin_(m, n, i) *
                                       ysin_(m, n, j) * zcos_(m, n, k);
          force_tmp_(m, 1, k, j, i) += ysss_.d_view(n) * xsin_(m, n, i) *
                                       ysin_(m, n, j) * zsin_(m, n, k);

          force_tmp_(m, 2, k, j, i) += zccc_.d_view(n) * xcos_(m, n, i) *
                                       ycos_(m, n, j) * zcos_(m, n, k);
          force_tmp_(m, 2, k, j, i) += zccs_.d_view(n) * xcos_(m, n, i) *
                                       ycos_(m, n, j) * zsin_(m, n, k);
          force_tmp_(m, 2, k, j, i) += zcsc_.d_view(n) * xcos_(m, n, i) *
                                       ysin_(m, n, j) * zcos_(m, n, k);
          force_tmp_(m, 2, k, j, i) += zcss_.d_view(n) * xcos_(m, n, i) *
                                       ysin_(m, n, j) * zsin_(m, n, k);
          force_tmp_(m, 2, k, j, i) += zscc_.d_view(n) * xsin_(m, n, i) *
                                       ycos_(m, n, j) * zcos_(m, n, k);
          force_tmp_(m, 2, k, j, i) += zscs_.d_view(n) * xsin_(m, n, i) *
                                       ycos_(m, n, j) * zsin_(m, n, k);
          force_tmp_(m, 2, k, j, i) += zssc_.d_view(n) * xsin_(m, n, i) *
                                       ysin_(m, n, j) * zcos_(m, n, k);
          force_tmp_(m, 2, k, j, i) += zsss_.d_view(n) * xsin_(m, n, i) *
                                       ysin_(m, n, j) * zsin_(m, n, k);
        });
  }
...
TaskStatus TurbulenceDriver::AddForcing(Driver *pdrive, int stage) {
  ...
  par_for(
      ...
      KOKKOS_LAMBDA(int m, int k, int j, int i) {
        force_(m, 0, k, j, i) =
            fcorr * force_(m, 0, k, j, i) + gcorr * force_tmp_(m, 0, k, j, i);
        force_(m, 1, k, j, i) =
            fcorr * force_(m, 1, k, j, i) + gcorr * force_tmp_(m, 1, k, j, i);
        force_(m, 2, k, j, i) =
            fcorr * force_(m, 2, k, j, i) + gcorr * force_tmp_(m, 2, k, j, i);
      });
  ...
  if (flag_relativistic) {
    ...
    Kokkos::parallel_reduce(
        ...
        KOKKOS_LAMBDA(const int &idx, Real &sum_t0, Real &sum_t1, Real &sum_t2,
          ...
          auto u_t = sqrt(1. + u0(m, IVX, k, j, i) * u0(m, IVX, k, j, i) +
    ...
    if (pmy_pack->pmhd != nullptr) {
      ...
      par_for(
          ...
          KOKKOS_LAMBDA(int m, int k, int j, int i) {
            u0(m, IEN, k, j, i) =
                fmin(u0(m, IEN, k, j, i), 40. * u0(m, IDN, k, j, i));
            ...
            u0(m, IEN, k, j, i) =
                uA_0 * en - uA_0 * (sx * vx + sy * vy + sz * vz);
            u0(m, IEN, k, j, i) -= u0(m, IDN, k, j, i);

            u0(m, IM1, k, j, i) = sx + (uA_0 - 1.) / (betaA * betaA) *
                                           (sx * vx + sy * vy + sz * vz) * vx;
            u0(m, IM2, k, j, i) = sy + (uA_0 - 1.) / (betaA * betaA) *
                                           (sx * vx + sy * vy + sz * vz) * vy;
            u0(m, IM3, k, j, i) = sz + (uA_0 - 1.) / (betaA * betaA) *
                                           (sx * vx + sy * vy + sz * vz) * vz;
