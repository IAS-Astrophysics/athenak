...
SourceTerms::SourceTerms(std::string block, MeshBlockPack *pp,
                         ParameterInput *pin)
    : pmy_pack(pp), source_terms_enabled(false) {
...
void SourceTerms::AddConstantAccel(DvceArray5D<Real> &u0,
  ...
  par_for(
      "const_acc", DevExeSpace(), 0, nmb1, ks, ke, js, je, is, ie,
...
void SourceTerms::AddShearingBox(DvceArray5D<Real> &u0,
                                 const DvceArray5D<Real> &w0, const Real bdt) {
  auto &indcs = pmy_pack->pmesh->mb_indcs;
  int is = indcs.is, ie = indcs.ie;
  int js = indcs.js, je = indcs.je;
  int ks = indcs.ks, ke = indcs.ke;
  ...
  //  Terms are implemented with orbital advection, so that v3 represents the
  //  perturbation from the Keplerian flow v_{K} = - q \Omega x
  Real &omega0_ = omega0;
  Real &qshear_ = qshear;
  Real qo = qshear * omega0;

  par_for(
      "sbox", DevExeSpace(), 0, nmb1, ks, ke, js, je, is, ie,
      KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
        ...
        if ((u0.extent_int(1) - 1) == IEN) {
          u0(m, IEN, k, j, i) += qo * bdt * (mom1 * mom3 / den);
        }
      });
}
  ...
  par_for(
      ...
      KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
        ...
        if ((u0.extent_int(1) - 1) == IEN) {
          u0(m, IEN, k, j, i) -=
              qo * bdt *
              (bcc0(m, IBX, k, j, i) * bcc0(m, IBZ, k, j, i) -
               mom1 * mom3 / den);
        }
      });
...
//  background orbital velocity v_{K} = - q \Omega x in the toriodal (\phi or y)
//  direction See SG eqs. [49-52] (eqs for orbital advection), and [60]
...
void SourceTerms::AddSBoxEField(const DvceFaceFld4D<Real> &b0,
  ...
  if (pmy_pack->pmesh->two_d) {
    ...
    auto b2 = b0.x2f;
    par_for_outer(
        "acc0", DevExeSpace(), scr_size, scr_level, 0, nmb1, js, je + 1,
...
//! \brief Add explict ISM cooling and heating source terms in the energy
//! equations.
// NOTE source terms must all be computed using primitive (w0) and NOT conserved
// (u0) vars

void SourceTerms::AddISMCooling(DvceArray5D<Real> &u0,
                                const DvceArray5D<Real> &w0,
  ...
  Real cooling_unit = pmy_pack->punit->pressure_cgs() /
                      pmy_pack->punit->time_cgs() / n_unit / n_unit;
  Real heating_unit =
      pmy_pack->punit->pressure_cgs() / pmy_pack->punit->time_cgs() / n_unit;

  par_for(
      "cooling", DevExeSpace(), 0, nmb1, ks, ke, js, je, is, ie,
      KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
...
void SourceTerms::AddRelCooling(DvceArray5D<Real> &u0,
  ...
  par_for(
      ...
      KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
        ...
        u0(m, IEN, k, j, i) -= bdt * w0(m, IDN, k, j, i) * ut *
                               pow((temp * cooling_rate), cooling_power);
        u0(m, IM1, k, j, i) -= bdt * w0(m, IDN, k, j, i) * ux *
                               pow((temp * cooling_rate), cooling_power);
        u0(m, IM2, k, j, i) -= bdt * w0(m, IDN, k, j, i) * uy *
                               pow((temp * cooling_rate), cooling_power);
        u0(m, IM3, k, j, i) -= bdt * w0(m, IDN, k, j, i) * uz *
                               pow((temp * cooling_rate), cooling_power);
      });
  return;
}
...
void SourceTerms::AddBeamSource(DvceArray5D<Real> &i0, const Real bdt) {
  ...
  Real &dii_dt_ = dii_dt;
  par_for(
      "beam_source", DevExeSpace(), 0, nmb1, 0, nang1, ks, ke, js, je, is, ie,
