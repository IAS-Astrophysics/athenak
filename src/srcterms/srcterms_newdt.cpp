...
//! \brief function to compute timestep for source terms across all MeshBlock(s)
//! in a MeshBlockPack
...
void SourceTerms::NewTimeStep(const DvceArray5D<Real> &w0,
  ...
  if (rel_cooling) {
    ...
    Kokkos::parallel_reduce(
        ...
        KOKKOS_LAMBDA(const int &idx, Real &min_dt) {
          ...
          // add a tiny number
          Real cooling_heating =
              FLT_MIN + fabs(w0(m, IDN, k, j, i) * ut *
