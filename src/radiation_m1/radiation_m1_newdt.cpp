...
TaskStatus RadiationM1::NewTimeStep(Driver *pdrive, int stage) {
  ...
  Kokkos::parallel_reduce("radiation_m1_newdt",
  Kokkos::RangePolicy<>(DevExeSpace(), 0, nmb1),
  KOKKOS_LAMBDA(const int &m, Real &min_dt1, Real &min_dt2, Real &min_dt3) {
    min_dt1 = Kokkos::fmin(mbsize.d_view(m).dx1, min_dt1);
    min_dt2 = Kokkos::fmin(mbsize.d_view(m).dx2, min_dt2);
    min_dt3 = Kokkos::fmin(mbsize.d_view(m).dx3, min_dt3);
