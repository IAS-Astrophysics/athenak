...
//----------------------------------------------------------------------------------------
// \!fn void BoundaryValues::SendBuffersCC()
...
TaskStatus BoundaryValues::SendBuffersCC(DvceArray5D<Real> &a, int key)
{
  ...
  { 
    ...
    const int il = sbuf[n].index.d_view(0,0);
    const int iu = sbuf[n].index.d_view(0,1);
    const int jl = sbuf[n].index.d_view(0,2);
    const int ju = sbuf[n].index.d_view(0,3);
    const int kl = sbuf[n].index.d_view(0,4);
    const int ku = sbuf[n].index.d_view(0,5);
...
//----------------------------------------------------------------------------------------
// \!fn void BoundaryValues::SendBuffersFC()
...
TaskStatus BoundaryValues::SendBuffersFC(DvceFaceFld4D<Real> &b, int key)
{
  // create local references for variables in kernel
  int nmb = pmy_pack->pmb->nmb;
  // TODO: following only works when all MBs have the same number of neighbors
    ...
    Kokkos::parallel_for(Kokkos::TeamThreadRange<>(tmember, nkj), [&](const int idx)
    {
      ...
      // else copy field components into send buffer for MPI communication below
        ...
        if (v==0) {
          Kokkos::parallel_for(Kokkos::ThreadVectorRange(tmember,il,iu+1),[&](const int i)
          {
            sbuf[nn].data(m, v, (i-il) + ni*(j-jl + nj*(k-kl))) = b.x1f(m,k,j,i);
          });
        } else if (v==1) {
          Kokkos::parallel_for(Kokkos::ThreadVectorRange(tmember,il,iu+1),[&](const int i)
          {
            sbuf[nn].data(m, v, (i-il) + ni*(j-jl + nj*(k-kl))) = b.x2f(m,k,j,i);
          });
        } else {
          Kokkos::parallel_for(Kokkos::ThreadVectorRange(tmember,il,iu+1),[&](const int i)
          {
            sbuf[nn].data(m, v, (i-il) + ni*(j-jl + nj*(k-kl))) = b.x3f(m,k,j,i);
          });
        }
      }
    });
