...
void MeshBoundaryValues::RadiationM1BCs(MeshBlockPack *ppack, DualArray2D<Real> i_in,
                                      DvceArray5D<Real> i0) {
  ...
  if (pm->mesh_bcs[BoundaryFace::inner_x1] != BoundaryFlag::periodic) {
    int &is = indcs.is;
    int &ie = indcs.ie;
    par_for("radiationm1bc_x1", DevExeSpace(), 0,(nmb-1),0,(nvar-1),0,(n3-1),0,(n2-1),
    KOKKOS_LAMBDA(int m, int n, int k, int j) {
      // apply physical boundaries to inner_x1
      switch (mb_bcs.d_view(m,BoundaryFace::inner_x1)) {
        case BoundaryFlag::outflow:
          for (int i=0; i<ng; ++i) {
            i0(m,n,k,j,is-i-1) = rad_E_floor;
          }
          break;
        default:
          break;
