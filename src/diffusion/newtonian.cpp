...
//========================================================================================
//! \file newtonian.cpp
//  \brief iDerived class for isotropic viscosity in a Newtonian fluid (where viscous
...
Newtonian::Newtonian(MeshBlockPack *pp, ParameterInput *pin)
  : Viscosity(pp, pin)
{
  nu_iso = pin->GetReal("mhd","eta_ohm");
}
...
TaskStatus Newtonian::AddViscousFlux(const DvceArray5D<Real> &w0,DvceFaceFld5D<Real> &flx)
{
  ...
  if (!(nx2gt1)) {return TaskStatus::complete;}
  ...
  par_for_outer("visc1",DevExeSpace(), scr_size, scr_level, 0, nmb1, ks, ke, js, je+1,
    KOKKOS_LAMBDA(TeamMember_t member, const int m, const int k, const int j)
    {
      ScrArray1D<Real> fvx(member.team_scratch(scr_level), ncells1);
      ScrArray1D<Real> fvy(member.team_scratch(scr_level), ncells1);
  ...
  if (!(nx3gt1)) {return TaskStatus::complete;}
  ...
  par_for_outer("visc1",DevExeSpace(), scr_size, scr_level, 0, nmb1, ks, ke+1, js, je,
    KOKKOS_LAMBDA(TeamMember_t member, const int m, const int k, const int j)
    {
      ScrArray1D<Real> fvx(member.team_scratch(scr_level), ncells1);
      ScrArray1D<Real> fvy(member.team_scratch(scr_level), ncells1);
