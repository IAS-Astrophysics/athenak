...
//========================================================================================
//! \file radiation_femn_bcs.cpp
//  \brief apply outflow BCs for radiation_femn
...
void RadiationFEMNBCs(MeshBlockPack *ppack, DualArray2D<Real> i_in,
  ...
  if (pm->mesh_bcs[BoundaryFace::inner_x1] != BoundaryFlag::periodic) {
    int &is = indcs.is;
    int &ie = indcs.ie;
    par_for("radiationbc_x1",
            DevExeSpace(), 0, (nmb - 1), 0, (nvar - 1), 0, (n3 - 1), 0, (n2 - 1),
            KOKKOS_LAMBDA(int m, int n, int k, int j) {
              // apply physical boundaries to inner_x1
              switch (mb_bcs.d_view(m, BoundaryFace::inner_x1)) {
                case BoundaryFlag::outflow:
                  for (int i = 0; i < ng; ++i) {
                    i0(m, n, k, j, is - i - 1) = floor;
                  }
                  break;
                default:break;
              }
  ...
  if (pm->mesh_bcs[BoundaryFace::inner_x2] != BoundaryFlag::periodic) {
    int &js = indcs.js;
    int &je = indcs.je;
    par_for("radiationbc_x2", DevExeSpace(), 0, (nmb - 1), 0, (nvar - 1), 0, (n3 - 1),
            0, (n1 - 1),
  ...
  int &ks = indcs.ks;
  int &ke = indcs.ke;
  par_for("radiationbc_x3", DevExeSpace(), 0, (nmb - 1), 0, (nvar - 1), 0, (n2 - 1),
          0, (n1 - 1),
