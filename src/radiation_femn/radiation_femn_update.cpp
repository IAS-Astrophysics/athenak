...
TaskStatus RadiationFEMN::ExpRKUpdate(Driver *pdriver, int stage) {
  const int NGHOST = 2;
  const int tot_iter = 10 * num_points;
                ...
                KOKKOS_LAMBDA(TeamMember_t member, int m, int nuen, int k, int j, int i) {
                  ...
                  if (rad_mask_array_(m, k, j, i)) {
                    ...
                    for (int a = 1; a < 4; ++a) {
                      dg4_ddd(0, a, 0) = adm.g_dd(m, 0, 0, k, j, i) * dtbetax_du + adm.g_dd(m, 0, 1, k, j, i) * dtbetay_du + adm.g_dd(m, 0, 2, k, j, i) * dtbetaz_du
                                         + dtg_dd(a - 1, 0) * adm.beta_u(m, 0, k, j, i) + dtg_dd(a - 1, 1) * adm.beta_u(m, 1, k, j, i) +
                                         dtg_dd(a - 1, 2) * adm.beta_u(m, 2, k, j, i);
                    }
                    for (int a = 1; a < 4; ++a) {
                      for (int b = 1; b < 4; ++b) {
                  ...
                  if (num_energy_bins_ > 1)
                  {
                    ...
                    for (int index = 0; index < tot_iter; index++) {
                      ...
                      if (n_iter >= tot_iter) {
                        printf("Warning! BiCGSTAB routine exceeded total number of iterations without converging!\n");
                      }
                      ...
                      for (int index2 = 0; index2 < num_points_; index2++) {
                        p0(index2) = r0(index2) + beta * (p0(index2) - omega * v_arr(index2));
                      }
                    }
                    member.team_barrier();
