...
TaskStatus RadiationFEMN::TetradOrthogonalize(Driver *pdriver, int stage) {
  ...
  par_for("radiation_femn_tetrad_compute_L_mu_1", DevExeSpace(), 0, nmb1, 0, 3, ks, ke, js, je, is, ie,
          KOKKOS_LAMBDA(int m, int mu, int k, int j, int i) {
            L_mu_muhat0_(m, mu, 1, k, j, i) = (mu == 1) + (g_dd_(m, 1, 0, k, j, i) * L_mu_muhat0_(m, 0, 0, k, j, i) + g_dd_(m, 1, 1, k, j, i) * L_mu_muhat0_(m, 1, 0, k, j, i)
                + g_dd_(m, 1, 2, k, j, i) * L_mu_muhat0_(m, 2, 0, k, j, i) + g_dd_(m, 1, 3, k, j, i) * L_mu_muhat0_(m, 3, 0, k, j, i)) * L_mu_muhat0_(m, mu, 0, k, j, i);
          });
  ...
  par_for_outer("radiation_femn_tetrad_normalize_L_mu_1", DevExeSpace(), scr_size, scr_level, 0, nmb1, ks, ke, js, je, is, ie,
                KOKKOS_LAMBDA(TeamMember_t member, int m, int k, int j, int i) {
                  ...
                  Kokkos::parallel_reduce(Kokkos::TeamVectorRange(member, 0, 16), [&](const int munu, Real &partial_sum) {
                    int mu = int(munu / 4);
                    int nu = munu - 4 * mu;
                    partial_sum += g_dd_(m, mu, nu, k, j, i) * L_mu_muhat0_(m, mu, 1, k, j, i) * L_mu_muhat0_(m, nu, 1, k, j, i);
                  }, L_mu_1_norm);
                  member.team_barrier();
  ...
  par_for("radiation_femn_tetrad_compute_L_mu_2", DevExeSpace(), 0, nmb1, 0, 3, ks, ke, js, je, is, ie,
          KOKKOS_LAMBDA(int m, int mu, int k, int j, int i) {
            L_mu_muhat0_(m, mu, 2, k, j, i) = (mu == 2) - (g_dd_(m, 2, 0, k, j, i) * L_mu_muhat0_(m, 0, 1, k, j, i) + g_dd_(m, 2, 1, k, j, i) * L_mu_muhat0_(m, 1, 1, k, j, i)
                + g_dd_(m, 2, 2, k, j, i) * L_mu_muhat0_(m, 2, 1, k, j, i) + g_dd_(m, 2, 3, k, j, i) * L_mu_muhat0_(m, 3, 1, k, j, i)) * L_mu_muhat0_(m, mu, 1, k, j, i)
                + (g_dd_(m, 2, 0, k, j, i) * L_mu_muhat0_(m, 0, 0, k, j, i) + g_dd_(m, 2, 1, k, j, i) * L_mu_muhat0_(m, 1, 0, k, j, i)
                    + g_dd_(m, 2, 2, k, j, i) * L_mu_muhat0_(m, 2, 0, k, j, i) + g_dd_(m, 2, 3, k, j, i) * L_mu_muhat0_(m, 3, 0, k, j, i)) * L_mu_muhat0_(m, mu, 0, k, j, i);
          });
  ...
  par_for_outer("radiation_femn_tetrad_normalize_L_mu_2", DevExeSpace(), scr_size, scr_level, 0, nmb1, ks, ke, js, je, is, ie,
                KOKKOS_LAMBDA(TeamMember_t member, int m, int k, int j, int i) {
                  ...
                  Kokkos::parallel_reduce(Kokkos::TeamVectorRange(member, 0, 16), [&](const int munu, Real &partial_sum) {
                    int mu = int(munu / 4);
                    int nu = munu - 4 * mu;
                    partial_sum += g_dd_(m, mu, nu, k, j, i) * L_mu_muhat0_(m, mu, 2, k, j, i) * L_mu_muhat0_(m, nu, 2, k, j, i);
                  }, L_mu_2_norm);
                  member.team_barrier();
  ...
  par_for("radiation_femn_tetrad_compute_L_mu_3", DevExeSpace(), 0, nmb1, 0, 3, ks, ke, js, je, is, ie,
          KOKKOS_LAMBDA(int m, int mu, int k, int j, int i) {
            L_mu_muhat0_(m, mu, 3, k, j, i) = (mu == 3) - (g_dd_(m, 3, 0, k, j, i) * L_mu_muhat0_(m, 0, 1, k, j, i) + g_dd_(m, 3, 1, k, j, i) * L_mu_muhat0_(m, 1, 1, k, j, i)
                + g_dd_(m, 3, 2, k, j, i) * L_mu_muhat0_(m, 2, 1, k, j, i) + g_dd_(m, 3, 3, k, j, i) * L_mu_muhat0_(m, 3, 1, k, j, i)) * L_mu_muhat0_(m, mu, 1, k, j, i)
                - (g_dd_(m, 3, 0, k, j, i) * L_mu_muhat0_(m, 0, 2, k, j, i) + g_dd_(m, 3, 1, k, j, i) * L_mu_muhat0_(m, 1, 2, k, j, i)
                    + g_dd_(m, 3, 2, k, j, i) * L_mu_muhat0_(m, 2, 2, k, j, i) + g_dd_(m, 3, 3, k, j, i) * L_mu_muhat0_(m, 3, 2, k, j, i)) * L_mu_muhat0_(m, mu, 2, k, j, i)
                + (g_dd_(m, 3, 0, k, j, i) * L_mu_muhat0_(m, 0, 0, k, j, i) + g_dd_(m, 3, 1, k, j, i) * L_mu_muhat0_(m, 1, 0, k, j, i)
                    + g_dd_(m, 3, 2, k, j, i) * L_mu_muhat0_(m, 2, 0, k, j, i) + g_dd_(m, 3, 3, k, j, i) * L_mu_muhat0_(m, 3, 0, k, j, i)) * L_mu_muhat0_(m, mu, 0, k, j, i);
          });
  ...
  par_for_outer("radiation_femn_tetrad_normalize_L_mu_3", DevExeSpace(), scr_size, scr_level, 0, nmb1, ks, ke, js, je, is, ie,
                KOKKOS_LAMBDA(TeamMember_t member, int m, int k, int j, int i) {
                  ...
                  Kokkos::parallel_reduce(Kokkos::TeamVectorRange(member, 0, 16), [&](const int munu, Real &partial_sum) {
                    int mu = int(munu / 4);
                    int nu = munu - 4 * mu;
                    partial_sum += g_dd_(m, mu, nu, k, j, i) * L_mu_muhat0_(m, mu, 3, k, j, i) * L_mu_muhat0_(m, nu, 3, k, j, i);
                  }, L_mu_3_norm);
                  member.team_barrier();
