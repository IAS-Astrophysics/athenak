...
RadiationFEMN::RadiationFEMN(MeshBlockPack *ppack, ParameterInput *pin) :
    ...
    f1("f1", 1, 1, 1, 1, 1),
    radiation_mask("rad_mask", 1, 1, 1, 1),
    iflx("iflx", 1, 1, 1, 1, 1),
    ...
    VV_array("VVarray", 1),
    Veninv_matrix("Veninvmatrix", 1, 1),
  ...
  Ven_matrix(0,0) = 1.; // Fermi-Dirac basis eta = 0, T = 2
  VV_array(0) = 1.;
  Veninv_matrix(0,0) = 1.;
  Wen_matrix(0,0) = 1.;
  ...
  // reallocate radiation mask array (populate in pgen)
  Kokkos::realloc(radiation_mask, nmb, ncells3, ncells2, ncells1);
  Kokkos::deep_copy(radiation_mask, true);
