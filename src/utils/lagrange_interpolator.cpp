...
Real LagrangeInterpolator::Interpolate(AthenaTensor<Real, TensorSymm::NONE, 3, 1> &val,
                                        int nvars) {
  Real ivals = 0.;
  int &ng = pmy_pack->pmesh->mb_indcs.ng;
  int &ii0 = interp_indcs(0);
...
Real LagrangeInterpolator::ResetPointAndInterpolate(DvceArray5D<Real> &val, int nvars,
                                                    Real rcoords2[3]) {
  for(int i=0; i<3; ++i) {
    rcoord(i) = rcoords2[i];
  }
