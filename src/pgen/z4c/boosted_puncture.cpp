...
void ADMOnePunctureBoosted(MeshBlockPack *pmbp, ParameterInput *pin) {
  ...
  KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
    ...
    Real B0 = std::sqrt(std::pow(Gamma, 2) * (1 - std::pow(vel, 2) * std::pow(alpha0, 2) *
                                                      std::pow(psi0, -4)));
    ...
    adm.vK_dd(m, 0, 0, k, j, i) =
        Gamma * Gamma * B0 * x1v * vel / r0 * (2 * alpha0p - alpha0 / 2 * second_term);
    adm.vK_dd(m, 1, 1, k, j, i) =
        2 * Gamma * Gamma * x1v * vel * alpha0 * (-m0 / (2 * r0 * r0)) / (psi0 * B0 * r0);
