...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  if (verbose) {
    std::cout <<  "Initializing SGRID_DNSdata_Interpolate_ADMvars_to_xyz" << std::endl;
  }
  ...
  // Because SGRID only operates on the CPU, we can't construct the data on the GPU.
  ...
  if (verbose) std::cout << "SGRID freed." << std::endl;
  ...
  if (verbose) std::cout << "Data copied." << std::endl;
