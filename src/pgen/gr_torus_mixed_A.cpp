...
struct torus_pgen {
  ...
  Real potential_r_pow;                       // set how A_phi scales with r
  Real potential_rho_pow;                     // set how A_phi scales with rho
  Real potential_r_pow_tor;                   // set how A_theta scales with r
  Real potential_rho_pow_tor;                 // set how A_theta scales with rho
  Real potential_pol_frac;                    // set fraction of poloidal field
  Real potential_beta_min;                    // set magnetic field normalization
};
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  if (pmbp->pmhd != nullptr) {
    ...
    torus.potential_r_pow_tor = pin->GetOrAddReal("problem", "potential_r_pow_tor", 2.0);
    torus.potential_rho_pow_tor = pin->GetOrAddReal("problem", "potential_rho_pow_tor", 1.0);
  ...
  if (pgen.is_vertical_field) {
    ...
    if (r >= pgen.r_edge) {
      ...
      if (in_torus) {
        ...
        Real rho_cut = fmax(rho - pgen.potential_cutoff, 0.0);
        atheta_tilt = pow(rho_cut/pgen.rho_max, pgen.potential_rho_pow_tor)*scaling_param_tor;
        ...
        if (pgen.psi != 0.0) {
          ...
          Real dvarphi_dphi = sin_theta / SQR(sin_vartheta)
              * (pgen.cos_psi * sin_theta - pgen.sin_psi * cos_theta * cos_phi);
          // Unsure if this tilting accounting is correct here, do not use psi for now
