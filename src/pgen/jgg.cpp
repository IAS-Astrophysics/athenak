...
//========================================================================================
//! \file jgg.cpp
//! \brief Problem generator for linear MHD in in shearing sheet
//!
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  if (pmbp->phydro != nullptr) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
              << std::endl
              << "This problem works only for MHD." << std::endl;
    exit(EXIT_FAILURE);  
  }
  ...
  if (pmbp->pmhd != nullptr) {
    ...
    if(ipert == 1) {
      par_for("jgg1_c", DevExeSpace(), 0,(pmbp->nmb_thispack-1),ks,ke,js,je,is,ie,
      ...
      par_for("jgg1_b", DevExeSpace(), 0,(pmbp->nmb_thispack-1),ks,ke,js,je,is,ie,
      KOKKOS_LAMBDA(int m, int k, int j, int i) {
        ...
        Real b3 = amp*std::cos(ky*x2v);
        b0.x1f(m,k,j,i) = 0.0;
        b0.x2f(m,k,j,i) = 0.0;
        b0.x3f(m,k,j,i) = b3;

        if (eos.is_ideal) {
          u0(m,IEN,k,j,i) += 0.5*SQR(b3);
        }
      ...
      par_for("jgg2_c", DevExeSpace(), 0,(pmbp->nmb_thispack-1),ks,ke,js,je,is,ie,
      KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      par_for("jgg2_a1", DevExeSpace(), 0,(pmbp->nmb_thispack-1),ks,ke+1,js,je+1,is,ie,
      KOKKOS_LAMBDA(int m, int k, int j, int i) {
        Real &x1min = size.d_view(m).x1min;
        Real &x1max = size.d_view(m).x1max;
        Real x1v    = CellCenterX(i-is, nx1, x1min, x1max);
      ...
      par_for("jgg2_a2", DevExeSpace(), 0,(pmbp->nmb_thispack-1),ks,ke+1,js,je,is,ie+1,
      KOKKOS_LAMBDA(int m, int k, int j, int i) {
        Real &x1min = size.d_view(m).x1min;
        Real &x1max = size.d_view(m).x1max;
      ...
      par_for("jgg2_a3", DevExeSpace(), 0,(pmbp->nmb_thispack-1),ks,ke,js,je+1,is,ie+1,
      KOKKOS_LAMBDA(int m, int k, int j, int i) {
        Real &x1min = size.d_view(m).x1min;
        Real &x1max = size.d_view(m).x1max;
        Real x1f    = LeftEdgeX(i-is, nx1, x1min, x1max);
      ...
      par_for("jgg2_b1", DevExeSpace(), 0,(pmbp->nmb_thispack-1),ks,ke,js,je,is,ie+1,
      KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      par_for("jgg2_b2", DevExeSpace(), 0,(pmbp->nmb_thispack-1),ks,ke,js,je+1,is,ie,
      KOKKOS_LAMBDA(int m, int k, int j, int i) {
        // Compute face-centered fields from curl(A).
      ...
      par_for("jgg2_b3", DevExeSpace(), 0,(pmbp->nmb_thispack-1),ks,ke+1,js,je,is,ie,
      KOKKOS_LAMBDA(int m, int k, int j, int i) {
        // Compute face-centered fields from curl(A).
        Real dx1 = size.d_view(m).dx1;
        Real dx2 = size.d_view(m).dx2;
      ...
      if (eos.is_ideal) {
        par_for("jgg2_e", DevExeSpace(), 0,(pmbp->nmb_thispack-1),ks,ke,js,je,is,ie,
        KOKKOS_LAMBDA(int m, int k, int j, int i) {
