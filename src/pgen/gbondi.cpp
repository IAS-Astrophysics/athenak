...
//!   /GridData/Fields/VectorPotential_x, VectorPotential_y, VectorPotential_z - 3D
//!   A-field
//!
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  gizmo_disk.length_scale =
      pin->GetOrAddReal("problem", "length_scale", gizmo_disk.binary_separation);
  ...
  if (myrank == 0 && gizmo_disk.binary_data_available) {
    ...
    std::cout << "  BH1 position (a_bin=1): (" << gizmo_disk.bh1_pos[0] << ", "
              << gizmo_disk.bh1_pos[1] << ", " << gizmo_disk.bh1_pos[2] << ")"
              << std::endl;
    std::cout << "  BH2 position (a_bin=1): (" << gizmo_disk.bh2_pos[0] << ", "
              << gizmo_disk.bh2_pos[1] << ", " << gizmo_disk.bh2_pos[2] << ")"
              << std::endl;
    std::cout << "  Binary separation (input): " << gizmo_disk.binary_separation
              << std::endl;
    std::cout << "  BH1 position (scaled): ("
              << gizmo_disk.bh1_pos[0] * gizmo_disk.length_scale << ", "
              << gizmo_disk.bh1_pos[1] * gizmo_disk.length_scale << ", "
              << gizmo_disk.bh1_pos[2] * gizmo_disk.length_scale << ")" << std::endl;
    std::cout << "  BH2 position (scaled): ("
              << gizmo_disk.bh2_pos[0] * gizmo_disk.length_scale << ", "
  ...
  for (int i = 0; i < gizmo_disk.nx; ++i) {
    gizmo_disk.x_coords[i] =
        gizmo_disk.x_coords[i] * gizmo_disk.length_scale + gizmo_disk.x_offset;
  }
  for (int j = 0; j < gizmo_disk.ny; ++j) {
    gizmo_disk.y_coords[j] =
        gizmo_disk.y_coords[j] * gizmo_disk.length_scale + gizmo_disk.y_offset;
  }
  for (int k = 0; k < gizmo_disk.nz; ++k) {
    gizmo_disk.z_coords[k] =
        gizmo_disk.z_coords[k] * gizmo_disk.length_scale + gizmo_disk.z_offset;
  }
  ...
  for (int idx = 0; idx < ntot; ++idx) {
    ...
    if (gizmo_disk.temperature_as_eint) {
      ...
      gizmo_disk.internal_energy[idx] *=
          gizmo_disk.density[idx] * gizmo_disk.pressure_scale;
    }
  }

  if (myrank == 0) {
    std::cout << "GIZMO data: " << gizmo_disk.nx << " x " << gizmo_disk.ny << " x "
              << gizmo_disk.nz << std::endl;
    std::cout << "Domain: [" << gizmo_disk.x_min << ", " << gizmo_disk.x_max << "] x ["
              << gizmo_disk.y_min << ", " << gizmo_disk.y_max << "] x ["
              << gizmo_disk.z_min << ", " << gizmo_disk.z_max << "]" << std::endl;
  }
  ...
  if (is_mhd) {
    ...
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      auto interp_A =
        ...
        if (px < x_min || px > x_max || py < y_min || py > y_max || pz < z_min ||
            pz > z_max) {
          return 0.0;
        }
...
void ReadGizmoHDF5(const std::string &filename, GizmoDiskData &data) {
  ...
  if (eint_test >= 0) {
    ...
    if (myrank == 0) {
      std::cout << "  Note: Temperature data found, will convert to internal energy"
                << std::endl;
    }
  }
  ...
  if (myrank == 0) {
    std::cout << "  Loaded " << data.nx << "x" << data.ny << "x" << data.nz << " grid"
              << std::endl;
  }
}
