...
struct torus_pgen {
  ...
  Real potential_rho_cutoff;                  // sets region of torus to magnetize - pol.
  Real potential_pgas_cutoff;                 // sets region of torus to magnetize - tor.
  Real potential_r_pow_pol, potential_pow_pol;// set how vector potential scales - pol.
  Real potential_r_pow_tor, potential_pow_tor;// set how vector potential scales - tor.
  Real potential_pol_frac;                    // fraction of potential from pol. component
  Real potential_beta_min;                    // min ratio of gas to mag pressure
};
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  if (pmbp->pmhd != nullptr) {
    ...
    torus.potential_beta_min = pin->GetOrAddReal("problem", "potential_beta_min",100);
    torus.potential_pol_frac = pin->GetOrAddReal("problem", "potential_pol_frac",1.0);
    torus.potential_rho_cutoff = pin->GetOrAddReal("problem", "potential_rho_cutoff",0.2);
    torus.potential_pgas_cutoff = pin->GetReal("problem", "potential_pgas_cutoff");
    torus.potential_r_pow_pol = pin->GetOrAddReal("problem", "potential_r_pow_pol",0.0);
    torus.potential_pow_pol = pin->GetOrAddReal("problem", "potential_pow_pol",1.0);
    ...
    par_for("pgen_torus2", DevExeSpace(), 0,nmb-1,ks,ke,js,je,is,ie,
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      // cell-centered fields are simple linear averages of face-centered fields
...
static Real CalculateLFromRPeak(struct torus_pgen pgen, Real r) {
  ...
  Real num = sgn*(SQR(r*r) + SQR(pgen.spin*r) - 2.0*pgen.mass*SQR(pgen.spin)*r)
           - pgen.spin*(r*r - pgen.spin*pgen.spin)*sqrt(pgen.mass*r);
  Real denom = SQR(r) - 3.0*pgen.mass*r + sgn*2.0*pgen.spin*sqrt(pgen.mass*r);
...
Real A1(struct torus_pgen trs, Real x1, Real x2, Real x3) {
  ...
  if (r >= trs.r_edge) {
    ...
    if (log_h >= 0.0) {
      ...
      Real pgas_cutoff = fmax(pgas - trs.potential_pgas_cutoff, static_cast<Real>(0.0));
      aphi = pow(r, trs.potential_r_pow_pol) * pow(rho_cutoff, trs.potential_pow_pol);
      atheta = pow(r, trs.potential_r_pow_tor) * pow(pgas_cutoff, trs.potential_pow_tor);
    }
  }

  Real big_r = sqrt( SQR(x1) + SQR(x2) + SQR(x3) );
  Real sqrt_term =  2.0*SQR(r) - SQR(big_r) + SQR(trs.spin);
  Real sqrt_term_2 = sqrt(SQR(r)-SQR(x3));
  if (sqrt_term == 0.0 || (SQR(r)-SQR(x3)) <= 0.0) {
    return 0.0;
  }

  // dtheta/dx = (partial theta/partial r)(partial r/partial x)
  Real A1_tor = atheta * (x1*x3/(sqrt_term*sqrt_term_2));

  // dphi/dx =  partial phi/partial x + partial phi/partial r partial r/partial x
  Real A1_pol = aphi*(-x2/(SQR(x1)+SQR(x2)) + trs.spin*x1*r/((SQR(trs.spin)+SQR(r))*sqrt_term));
  return (A1_tor*(1.0-trs.potential_pol_frac) + A1_pol*trs.potential_pol_frac);
}
...
Real A2(struct torus_pgen trs, Real x1, Real x2, Real x3) {
  ...
  // dtheta/dy = (partial theta/partial r)(partial r/partial y)
  Real A2_tor = atheta * (x2*x3/(sqrt_term*sqrt_term_2));

  // dphi/dx =  partial phi/partial y + partial phi/partial r partial r/partial y
  Real A2_pol = aphi*( x1/(SQR(x1)+SQR(x2)) + trs.spin*x2*r/((SQR(trs.spin)+SQR(r))*sqrt_term) );
  return (A2_tor*(1.0-trs.potential_pol_frac) + A2_pol*trs.potential_pol_frac);
}
...
Real A3(struct torus_pgen trs, Real x1, Real x2, Real x3) {
  ...
  // dtheta/dz = partial theta/partial z + (partial theta/partial r)(partial r/partial y)
  Real A3_tor = atheta * ((SQR(r)+SQR(x3))/sqrt_term - 1.0)/sqrt_term_2;

  // dphi/dz =   partial phi/partial r partial r/partial z
  Real A3_pol = aphi * ( trs.spin*x3/(r*sqrt_term) );
  return (A3_tor*(1.0-trs.potential_pol_frac) + A3_pol*trs.potential_pol_frac);
}
