...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  /*
  std::cout << coordinate_ind[0] << "  " << coordinate_ind[1] << std::endl;
  Real x1v = CellCenterX(coordinate_ind[0], nx1, x1min, x1max);
  Real x2v = CellCenterX(coordinate_ind[1], nx2, x2min, x2max);
  std::cout << x1v << "   " << x2v << std::endl;
  */
  ...
  std::cout << "expected value  " << 1.000 + 0.2000*std::sin(5.000*M_PI*(x_interp[0]))*std::sin(3.0*M_PI*(x_interp[1]))*std::sin(3.0*M_PI*(x_interp[2])) << std::endl;
