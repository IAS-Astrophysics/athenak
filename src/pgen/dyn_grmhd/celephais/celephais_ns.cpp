...
void SetupNS(ParameterInput *pin, Mesh* pmy_mesh_) {
  ...
  const double mnuc_cgs   = bconfig.eos<double>(MNUC_CGS);
  ...
  for (int idx = 0; idx < width; ++idx) {
    ...
    const double x = static_cast<double>(CellCenterX(
        i - is, indcs.nx1, size.h_view(m).x1min, size.h_view(m).x1max));
    const double y = static_cast<double>(CellCenterX(
        j - js, indcs.nx2, size.h_view(m).x2min, size.h_view(m).x2max));
  ...
  Kokkos::parallel_for("celephais_fill",
    ...
    for (int lane = 0; lane < point_lane_width; ++lane) {
      ...
      points[lane].set(1) = static_cast<double>(x);
      points[lane].set(2) = static_cast<double>(y);
