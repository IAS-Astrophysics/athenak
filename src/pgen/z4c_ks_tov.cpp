...
void SetupTOV(ParameterInput* pin, Mesh* pmy_mesh_, tov_pgen& tov) {
  ...
  Real x1c = pin->GetOrAddInteger("problem", "star_center_x1", 0);
  Real x2c = pin->GetOrAddInteger("problem", "star_center_x2", 0);
  Real x3c = pin->GetOrAddInteger("problem", "star_center_x3", 0);
  ...
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    Real x1v = CellCenterX(i-is, indcs.nx1, x1min, x1max) - x1c;

    Real &x2min = size.d_view(m).x2min;
    Real &x2max = size.d_view(m).x2max;
    Real x2v = CellCenterX(j-js, indcs.nx2, x2min, x2max) - x2c;

    Real &x3min = size.d_view(m).x3min;
    Real &x3max = size.d_view(m).x3max;
    Real x3v = CellCenterX(k-ks, indcs.nx3, x3min, x3max) - x3c;
  ...
  if (pmbp->pmhd != nullptr) {
    ...
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      Real &x1max = size.d_view(m).x1max;
      int nx1 = indcs.nx1;
      Real x1v = CellCenterX(i-is, nx1, x1min, x1max) - x1c;
      Real x1f = LeftEdgeX(i-is,nx1,x1min,x1max) - x1c;

      Real &x2min = size.d_view(m).x2min;
      Real &x2max = size.d_view(m).x2max;
      int nx2 = indcs.nx2;
      Real x2v = CellCenterX(j-js, nx2, x2min, x2max) - x2c;
      Real x2f = LeftEdgeX(j-js,nx2,x2min,x2max) - x2c;

      Real &x3min = size.d_view(m).x3min;
      Real &x3max = size.d_view(m).x3max;
      int nx3 = indcs.nx3;
      Real x3v = CellCenterX(k-ks, nx3, x3min, x3max) - x3c;
      Real x3f = LeftEdgeX(k-ks,nx3,x3min,x3max) - x3c;
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  if (!pmbp->pcoord->is_dynamical_relativistic) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__ << std::endl
              << "superpose KS TOV star problem can only be run when <adm> block is present"
              << std::endl;
    exit(EXIT_FAILURE);
  }
  if (pmbp->pz4c == nullptr) {
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__ << std::endl
              << "superpose KS TOV star problem can only be run in Z4c, but no <z4c> block "
...
void ADMKerrSchild(MeshBlockPack *pmbp, ParameterInput *pin) {
  ...
  KOKKOS_LAMBDA(int m, int k, int j, int i) {
    ...
    Real alpha, betax, betay, betaz, psi4;
    ...
    adm.beta_u(m,0,k,j,i) += betax;
    adm.beta_u(m,1,k,j,i) += betay;
    adm.beta_u(m,2,k,j,i) += betaz;
    adm.g_dd(m,0,0,k,j,i) += gxx-1;
    adm.g_dd(m,0,1,k,j,i) += gxy;
    adm.g_dd(m,0,2,k,j,i) += gxz;
    adm.g_dd(m,1,1,k,j,i) += gyy-1;
    adm.g_dd(m,1,2,k,j,i) += gxz;
    adm.g_dd(m,2,2,k,j,i) += gzz-1;
    adm.vK_dd(m,0,0,k,j,i)+= Kxx;
    adm.vK_dd(m,0,1,k,j,i)+= Kxy;
    adm.vK_dd(m,0,2,k,j,i)+= Kxz;
    adm.vK_dd(m,1,1,k,j,i)+= Kyy;
    adm.vK_dd(m,1,2,k,j,i)+= Kyz;
    adm.vK_dd(m,2,2,k,j,i)+= Kzz;
    Real det = adm::SpatialDet(
            adm.g_dd(m,0,0,k,j,i), adm.g_dd(m,0,1,k,j,i),
            adm.g_dd(m,0,2,k,j,i), adm.g_dd(m,1,1,k,j,i),
            adm.g_dd(m,1,2,k,j,i), adm.g_dd(m,2,2,k,j,i));
