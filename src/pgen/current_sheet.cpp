  ...
  if (pmbp->pmhd != nullptr) {
    ...
    par_for("pgen_mhd", DevExeSpace(), 0,(pmbp->nmb_thispack-1),ks,ke,js,je,is,ie,
      KOKKOS_LAMBDA(int m, int k, int j, int i)
      {
        ...
        if (i==ie) {
          b0.x1f(m,k,j,i+1) = bb0*epsb*std::sin(kval*x2v)*(exp(-1.0*pow((x1fp1+x01)/a0,2.0))+exp(-1.0*pow((x1fp1-x01)/a0,2.0))); 
        }
        if (j==je) {
          b0.x2f(m,k,j+1,i) = bb0*(tanh((x1v+x01)/a0))- bb0*(tanh((x1v-x01)/a0))-bb0-bb0*epsb*2.0*cos(kval*x2fp1)*( exp(-1.0*pow((x1v+x01)/a0,2.0))*(x1v+x01)+exp(-1.0*pow((x1v-x01)/a0,2.0))*(x1v-x01) )/(kval*a0*a0); 
        }
        if (k==ke) {
          b0.x3f(m,k+1,j,i) = bg; 
        }
