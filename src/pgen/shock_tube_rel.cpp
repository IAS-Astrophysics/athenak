...
void ProblemGenerator::ShockTube_SR_(MeshBlock *pmb, ParameterInput *pin)
{
    ...
    case 1:
      par_for("pgen_shock_tube1", pmb->exe_space, ks, ke, js, je, is, ie,
	    ...
	    Real wgas = rho + gamma_adi / gm1 * pgas;
            ...
            u0(IDN,k,j,i) = rho * gamma;
            u0(IM1,k,j,i) = wgas * gamma_sq * wl[IVX];
            u0(IM2,k,j,i) = wgas * gamma_sq * wl[IVY];
            u0(IM3,k,j,i) = wgas * gamma_sq * wl[IVZ];
            u0(IEN,k,j,i) = wgas * gamma_sq  - pgas - rho*gamma; //rho_eps * gamma_sq + (pgas + rho*gamma/(gamma+1.))*(v_sq*gamma_sq);
          } else {
	    ...
	    Real wgas = rho + gamma_adi / gm1 * pgas;
            ...
            u0(IDN,k,j,i) = rho * gamma;
            u0(IM1,k,j,i) = wgas * gamma_sq * wr[IVX];
            u0(IM2,k,j,i) = wgas * gamma_sq * wr[IVY];
            u0(IM3,k,j,i) = wgas * gamma_sq * wr[IVZ];
            u0(IEN,k,j,i) = wgas * gamma_sq  - pgas - rho*gamma; //rho_eps * gamma_sq + (pgas + rho*gamma/(gamma+1.))*(v_sq*gamma_sq);
          }
        }
      );
    ...
    case 2:
      par_for("pgen_shock_tube2", pmb->exe_space, ks, ke, js, je, is, ie,
	    ...
	    Real wgas = rho + gamma_adi / gm1 * pgas;
            ...
            u0(IDN,k,j,i) = rho * gamma;
            u0(IM2,k,j,i) = wgas * gamma_sq * wl[IVX];
            u0(IM3,k,j,i) = wgas * gamma_sq * wl[IVY];
            u0(IM1,k,j,i) = wgas * gamma_sq * wl[IVZ];
            u0(IEN,k,j,i) = wgas * gamma_sq  - pgas - rho*gamma; //rho_eps * gamma_sq + (pgas + rho*gamma/(gamma+1.))*(v_sq*gamma_sq);
          } else {
	    Real v_sq = SQR(wr[IVX]) + SQR(wr[IVY]) + SQR(wr[IVZ]);
	    Real gamma_sq = 1./(1.-v_sq);
	    ...
	    Real wgas = rho + gamma_adi / gm1 * pgas;
            ...
            u0(IDN,k,j,i) = rho * gamma;
            u0(IM2,k,j,i) = wgas * gamma_sq * wr[IVX];
            u0(IM3,k,j,i) = wgas * gamma_sq * wr[IVY];
            u0(IM1,k,j,i) = wgas * gamma_sq * wr[IVZ];
            u0(IEN,k,j,i) = wgas * gamma_sq  - pgas - rho*gamma; //rho_eps * gamma_sq + (pgas + rho*gamma/(gamma+1.))*(v_sq*gamma_sq);
          }
        }
      );
    ...
    case 3:
      par_for("pgen_shock_tube3", pmb->exe_space, ks, ke, js, je, is, ie,
	    ...
	    Real wgas = rho + gamma_adi / gm1 * pgas;
            ...
            u0(IDN,k,j,i) = rho * gamma;
            u0(IM3,k,j,i) = wgas * gamma_sq * wl[IVX];
            u0(IM1,k,j,i) = wgas * gamma_sq * wl[IVY];
            u0(IM2,k,j,i) = wgas * gamma_sq * wl[IVZ];
            u0(IEN,k,j,i) = wgas * gamma_sq  - pgas - rho*gamma; //rho_eps * gamma_sq + (pgas + rho*gamma/(gamma+1.))*(v_sq*gamma_sq);
          } else {
	    Real v_sq = SQR(wr[IVX]) + SQR(wr[IVY]) + SQR(wr[IVZ]);
	    Real gamma_sq = 1./(1.-v_sq);
	    ...
	    Real wgas = rho + gamma_adi / gm1 * pgas;
            ...
            u0(IDN,k,j,i) = rho * gamma;
            u0(IM3,k,j,i) = wgas * gamma_sq * wr[IVX];
            u0(IM1,k,j,i) = wgas * gamma_sq * wr[IVY];
            u0(IM2,k,j,i) = wgas * gamma_sq * wr[IVZ];
            u0(IEN,k,j,i) = wgas * gamma_sq  - pgas - rho*gamma; //rho_eps * gamma_sq + (pgas + rho*gamma/(gamma+1.))*(v_sq*gamma_sq);
          }
        }
      );
