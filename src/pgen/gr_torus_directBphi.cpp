...
//! Simplified and implemented in Kokkos by JMS. This version is for purely a toroidal
//! field configuration, written by GH.
//!
//! References:
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  if (pmbp->pmhd != nullptr) {
    ...
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      b0.x1f(m,k,j,i) = 0.5*(bcc_(m,IBX,k,j,i-1) + bcc_(m,IBX,k,j,i));
      b0.x2f(m,k,j,i) = 0.5*(bcc_(m,IBY,k,j-1,i) + bcc_(m,IBY,k,j,i));
      b0.x3f(m,k,j,i) = 0.5*(bcc_(m,IBZ,k-1,j,i) + bcc_(m,IBZ,k,j,i));

      // When neighboring MeshBock is at finer level, compute magnetic field as sum of
      // values at fine grid resolution.  This guarantees flux on shared fine/coarse
      // faces is identical.

      // Correct B1 at x2-faces, x3-faces, and x2x3-edges
      if ((nghbr.d_view(m,8 ).lev > mblev.d_view(m) && j==js) ||
        ...
        b0.x1f(m,k,j,i) = 0.5*(B1(trs, xl,x2f,x3f) + B1(trs, xr,x2f,x3f));
      }
      ...
      if ((nghbr.d_view(m,0 ).lev > mblev.d_view(m) && i==is) ||
        ...
        b0.x2f(m,k,j,i) = 0.5*(B2(trs, x1f,xl,x3f) + B2(trs, x1f,xr,x3f));
      }
      ...
      if ((nghbr.d_view(m,0 ).lev > mblev.d_view(m) && i==is) ||
        ...
        b0.x3f(m,k,j,i) = 0.5*(B3(trs, x1f,x2f,xl) + B3(trs, x1f,x2f,xr));
      }

      // Include extra face-component at edge of block in each direction
      if (i==ie) {
        b0.x1f(m,k,j,i+1) = 0.5*(bcc_(m,IBX,k,j,i) + bcc_(m,IBX,k,j,i+1));
      }
      if (j==je) {
        b0.x2f(m,k,j+1,i) = 0.5*(bcc_(m,IBY,k,j,i) + bcc_(m,IBY,k,j+1,i));
      }
      if (k==ke) {
        b0.x3f(m,k+1,j,i) = 0.5*(bcc_(m,IBZ,k,j,i) + bcc_(m,IBZ,k+1,j,i));
      }
    });
...
//----------------------------------------------------------------------------------------
// Function for calculating phi magnetic field in Spherical KS given CKS coordinates
// Inputs:
//   x1,x2,x3: Cartesian Kerr-Schild coordinates of point
// Outputs:
//   pbtheta,pbphi: pointers to lower theta, phi components in desired coordinates

KOKKOS_INLINE_FUNCTION
static void CalculateFieldInTiltedTorus(struct torus_pgen pgen,
                                                  Real r, Real theta, Real phi,
                                                  Real *pbtheta, Real *pbphi) {
  // Find field components, accounting for tilt
...
//----------------------------------------------------------------------------------------
// Function to compute 1-component of field.  First computes phi-componenent
...
Real B1(struct torus_pgen pgen, Real x1, Real x2, Real x3) {
  ...
  // calculate field in spherical KS
  Real btheta, bphi;
...
//----------------------------------------------------------------------------------------
// Function to compute 2-component of field. See comments for B1.
...
//----------------------------------------------------------------------------------------
// Function to compute 3-component of field. See comments for field.
