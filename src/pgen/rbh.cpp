...
struct torus_pgen {
  ...
  bool boost_flag = false;                  // flag to indicate boosted torus
  Real boost_velocity;                      // boost velocity magnitude
  Real boost_angle;                         // boost angle relative to x-y plane (radians)
  Real boost_vx, boost_vy, boost_vz;        // boost velocity components
  Real boost_gamma;                         // Lorentz factor of boost
};
...
void ProblemGenerator::UserProblem(ParameterInput *pin, const bool restart) {
  ...
  if (torus.boost_flag) {
    torus.boost_velocity = pin->GetReal("problem", "boost_velocity");
    torus.boost_angle = pin->GetReal("problem", "boost_angle") * (M_PI/180.0); // convert to radians
    
    // Calculate boost velocity components
    // boost_angle is measured from x-y plane
    // positive angle means boosting upward (toward +z)
    Real v_xy = torus.boost_velocity * cos(torus.boost_angle);
    torus.boost_vz = torus.boost_velocity * sin(torus.boost_angle);
    
    // Read azimuthal direction in x-y plane (default: along x-axis)
    Real boost_phi = pin->GetOrAddReal("problem", "boost_phi", 0.0) * (M_PI/180.0);
    torus.boost_vx = v_xy * cos(boost_phi);
    torus.boost_vy = v_xy * sin(boost_phi);
    
    // Calculate Lorentz factor
    torus.boost_gamma = 1.0 / sqrt(1.0 - SQR(torus.boost_velocity));
    
    if (global_variable::my_rank == 0) {
      std::cout << "Disk boost enabled:" << std::endl;
      std::cout << "  Boost velocity = " << torus.boost_velocity << std::endl;
      std::cout << "  Boost angle = " << torus.boost_angle * (180.0/M_PI) << " deg" << std::endl;
      std::cout << "  Boost phi = " << boost_phi * (180.0/M_PI) << " deg" << std::endl;
      std::cout << "  Boost gamma = " << torus.boost_gamma << std::endl;
      std::cout << "  Boost velocity components: vx=" << torus.boost_vx 
                << ", vy=" << torus.boost_vy 
                << ", vz=" << torus.boost_vz << std::endl;
    }
  }

  // Apply boost after restart
  if (restart && torus.boost_flag) {
    ...
    // Apply Lorentz boost to the fluid velocities
    par_for("apply_boost", DevExeSpace(), 0, nmb-1, ks, ke, js, je, is, ie,
    KOKKOS_LAMBDA(int m, int k, int j, int i) {
      ...
      Real lor = sqrt(1.0 + q);
      Real u0 = lor / alpha;
      Real u1 = uu1 - alpha * lor * gupper[0][1];
      Real u2 = uu2 - alpha * lor * gupper[0][2];
      Real u3 = uu3 - alpha * lor * gupper[0][3];
      ...
      // Apply Lorentz boost: u'^mu = Lambda^mu_nu u^nu
      // For a boost with velocity v^i in flat space portion
      // We boost the spatial components
      Real gamma_b = trs.boost_gamma;
      Real vb1 = trs.boost_vx;
      Real vb2 = trs.boost_vy;
      Real vb3 = trs.boost_vz;
      
      // Boost in the coordinate frame (approximate for small velocities)
      // Full relativistic velocity addition formula
      Real vdotu = vb1*uu1 + vb2*uu2 + vb3*uu3;
      
      // Boosted 3-velocity (using relativistic velocity addition)
      Real factor = gamma_b / (1.0 + gamma_b * vdotu);
      Real uu1_new = uu1 - vb1 + factor * vdotu * vb1;
      Real uu2_new = uu2 - vb2 + factor * vdotu * vb2;
      Real uu3_new = uu3 - vb3 + factor * vdotu * vb3;

      // Update primitive velocities
      w0_(m,IVX,k,j,i) = uu1_new;
      w0_(m,IVY,k,j,i) = uu2_new;
      w0_(m,IVZ,k,j,i) = uu3_new;
    });

    // Convert primitives back to conserved
    if (pmbp->padm == nullptr) {
      if (pmbp->phydro != nullptr) {
    ...
    if (global_variable::my_rank == 0) {
      std::cout << "Disk boost applied after restart." << std::endl;
    }
    
    return;
  }
  
  // return if restart (without boost)
  if (restart) return;  // Select either Hydro or MHD
  DvceArray5D<Real> u0_, w0_;
  if (pmbp->phydro != nullptr) {
    u0_ = pmbp->phydro->u0;
    w0_ = pmbp->phydro->w0;
