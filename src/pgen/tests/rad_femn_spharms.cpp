...
// ----------
  ...
  std::cout << "Tests for comparison of spherical harmonics (theta der)" << std::endl;
  for (int l = 0; l <= 2; ++l) {
    for (int m = -l; m <= l; m++) {
      Real phi = M_PI / 5.;
      Real theta = M_PI / 3.;
      ...
      std::cout << l << " " << m << " " << result_alt << " " << result << std::endl;
    }
  }
  std::cout << std::endl;
  ...
  std::cout << "Abs error (old): " << max_error_alt << ", (new) :" << max_error << std::endl;
  std::cout << std::endl;
