...
//! \brief Allocate arrays to save density flux over entire step for tracer particles
...
//! \brief Save IDN fluxes across all RK stages for full time step
...
TaskStatus Hydro::SaveFlux(Driver *pdrive, int stage) {
  if (uflxidn_saved) {
    ...
    KOKKOS_LAMBDA(const int m, const int k, const int j, const int i) {
      if (j <= je && k <= ke) {
        flxidn1(m,k,j,i) = gam0*flxidn1(m,k,j,i)
                          + flx1(m,IDN,k,j,i) / mbsize.d_view(m).dx1 * beta_dt;
      }

      if (multi_d && i <= ie && k <= ke) {
        flxidn2(m,k,j,i) = gam0*flxidn2(m,k,j,i)
                          + flx2(m,IDN,k,j,i) / mbsize.d_view(m).dx2 * beta_dt;
      }

      if (three_d && i <= ie && j <= je) {
        flxidn3(m,k,j,i) = gam0*flxidn3(m,k,j,i)
                          + flx3(m,IDN,k,j,i) / mbsize.d_view(m).dx3 * beta_dt;
      }
    });
  }
