  ...
  KOKKOS_LAMBDA(TeamMember_t member, const int m) {
    ...
    par_for_inner(member, 0, nangles-1, [&](const int n) {
      ...
      if (cartcoord.d_view(n,0) >= x1min && cartcoord.d_view(n,0) <= x1max 
        ...
        interp_indices.d_view(n,1) = (int) (cartcoord.d_view(n,0)-x1min-delta[0]/2)/delta[0];
        interp_indices.d_view(n,2) = (int) (cartcoord.d_view(n,1)-x2min-delta[1]/2)/delta[1];
        interp_indices.d_view(n,3) = (int) (cartcoord.d_view(n,2)-x3min-delta[2]/2)/delta[2];
      }
    });
  });
