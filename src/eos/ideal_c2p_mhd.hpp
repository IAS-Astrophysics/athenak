...
void SingleC2P_IdealMHD(MHDCons1D &u, const EOS_Data &eos,
  ...
  // apply entropy floor
  Real sfloor = eos.EntropyFloorLocal(w.d);
...
Real Equation44(const Real mu, const Real b2, const Real rpar, const Real r, const Real q,
  ...
  Real epsmin = fmax(eos.pfloor/(wd*gm1),
                     eos.EntropyFloorLocal(wd)*pow(wd, gm1)/gm1);
  eps = fmax(eps, epsmin);
...
void SingleC2P_IdealSRMHD(MHDCons1D &u, const EOS_Data &eos, Real s2, Real b2, Real rpar,
  ...
  Real epsmin = fmax(eos.pfloor/(dens*gm1),
                     eos.EntropyFloorLocal(dens)*pow(dens, gm1)/gm1);
  if (eps <= epsmin) {
    eps = epsmin;
    efloor_used = true;
  }
