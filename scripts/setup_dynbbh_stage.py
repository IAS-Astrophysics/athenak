...
SPIN_CASES = (
    ("spin_m0p7", 0.7, 180.0, 180.0, "anti-aligned"),
    ("spin_0", 0.0, 0.0, 0.0, "zero"),
    ("spin_p0p7", 0.7, 0.0, 0.0, "aligned"),
    ("spin_p0p7_tilt30", 0.7, 60.0, 0.0, "bh1_30deg_above_plane_bh2_aligned"),
)
...
if (( need_resubmit && AUTO_RESUBMIT )); then
  ...
  echo "All runs finished without wall-clock termination."
fi
""")
    path.chmod(0o755)
...
def stage3_spin_text(text, *, root, case, spin_name, spin_mag, theta1_deg,
    ...
    updates = {
        ...
        ("problem", "th_a1"): format_real(theta1_deg),
        ("problem", "th_a2"): format_real(theta2_deg),
        ("problem", "ph_a1"): "0.0",
        ("problem", "ph_a2"): "0.0",
...
def setup_zoom_survey(args):
    ...
    for source in case_sources:
        ...
        for spin_name, spin_mag, theta1_deg, theta2_deg, alignment in SPIN_CASES:
...
2. After the stage-2 runs have produced clean horizon restarts, inspect them and then
   ...
   to the latest stage-2 restart for the same SANE/MAD/BONDI case.  The fourth
   survey case uses `chi=0.7` on both holes with BH1 tilted 30 degrees above
   the orbital plane (`th_a1=60`) and BH2 aligned with the orbital angular
   momentum (`th_a2=0`).
   Stage 3 is configured as {args.stage3_max_iters} internal segment(s): PBS
   walltime `{args.stage3_walltime}` and application runtime `{args.runtime}`.
...
def main():
    ...
    parser.add_argument("--spin-orbits", type=float, default=40.0)
    ...
    parser.add_argument("--stage3-walltime", default="02:00:00")
    parser.add_argument("--stage3-job-name", default="zoom_spin")
    parser.add_argument("--stage3-max-iters", type=int, default=2)
