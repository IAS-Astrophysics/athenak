...
def parse_cli():
  ...
  p.add_argument("-fpath", type=str, required=True, help="/path/to/cce/h5/dumps")
  p.add_argument("-ftype", type=str, required=True, help="input file type:{h5,bin}")
...
def load(fpath: str, field_name: str, attrs: dict) -> list:
  ...
  if attrs["file_type"] == "h5":
    ...
    t = []
    for f in flist:
      nr, num_l_modes, time, rin, rout, data_real, data_imag, index_to_lm = read_cce_file(f)
      t.append(time)
...
def get_attribute(fpath: str,
  ...
  if type == "h5":
    ...
    for f in flist:
      nr, num_l_modes, time, rin, rout, data_real, data_imag, index_to_lm = read_cce_file(f)
      t.append(time)
      #data.append(data_real + 1j * data_imag)
    t = np.array(t)
    #data = np.array(data)
