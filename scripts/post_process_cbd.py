    ...
    try:
        with h5py.File(fname, "r") as f:
            ...
            for b in range(n_blocks):
                    ...
                    if np.any(mask_easy):
                        ...
                        if np.any(valid_easy):
                            ...
                            for v in range(n_vars):
                                ...
                                shell_integrals[v] += np.bincount(
                                    bins_to_count, weights=w, minlength=n_bins
                                )
                        ...
                        for ox, oy, oz in offsets:
                            ...
                            if np.any(mask_valid_sub):
                                ...
                                for v in range(n_vars):
                                    ...
                                    shell_integrals[v] += np.bincount(
                                        valid_bins, weights=w_sub, minlength=n_bins
                                    )

                # --- B. Excision Volume ---
                dist_sq_1 = (
                    (X - bh1_pos[0]) ** 2 + (Y - bh1_pos[1]) ** 2 + (Z - bh1_pos[2]) ** 2
                )
                ...
                dist_sq_2 = (
                    (X - bh2_pos[0]) ** 2 + (Y - bh2_pos[1]) ** 2 + (Z - bh2_pos[2]) ** 2
                )
                ...
                if use_supersampling:
                    ...
                    if np.any(mask_overlap):
                        ...
                        for ox, oy, oz in offsets:
                            R_sub = np.sqrt(
                                (X_ov + ox) ** 2 + (Y_ov + oy) ** 2 + (Z_ov + oz) ** 2
                            )
                            mask_sub_in = R_sub < rmin
                            if np.any(mask_sub_in):
                                for v in range(n_vars):
                                    cavity_vol[v] += (
                                        np.sum(data_ov[v, mask_sub_in]) * sub_dV
                                    )
                else:
...
def process_files(
    file_pattern, rmin, rmax, dr, params, output_filename, delete_source=False, nproc=1
):
    ...
    for f in all_files:
        ...
        if should_process:
            tasks.append(
                (f, npz_out, rmin, rmax, dr, sep, q, r_exc_1, r_exc_2, delete_source)
            )
        ...
        if nproc > 1:
            with Pool(nproc) as pool:
                for i, result in enumerate(pool.imap(integrate_and_save, tasks)):
                    ...
                    sys.stdout.write(
                        f"\r[Proc] {processed_count}/{total_tasks} | Last: {status} {msg[:40]}..."
                    )
                    sys.stdout.flush()
        else:
...
if __name__ == "__main__":
    ...
    process_files(
        "./bin/torus.angular_momentum.*.athdf",
        R_MIN,
        R_MAX,
        DR,
        params,
