...
def detect_ranks(base_dir):
    ...
    count = ranks[-1] + 1
    print(
        f"[Auto-Detect] Found {len(ranks)} rank directories (Max Rank ID: {ranks[-1]})."
    )
    return count
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--dir", type=str, default="rst", help="Directory containing rank_ folders"
    )
    parser.add_argument(
        "-f", "--file", type=str, required=True, help="Basename (e.g., torus.00000.rst)"
    )
    parser.add_argument(
        "-n",
        "--nprocs",
        type=int,
        help="Optional: Force number of ranks (overrides auto-detect)",
    )
