...
def process_frame(f_xy):
    ...
    try:
        ...
        for i, (field, cmap, (zmin, zmax), is_log, ptype) in enumerate(cols_config):
            ...
            # ------------------------------------
            slc_xy = yt.SlicePlot(
                ds_xy, "z", field, center=[0.0, 0.0, 0.0], width=(200.0, "code_length")
            )
            ...
            try:
                if ptype == 0:
                    ...
                    slc_xy.annotate_streamlines(
                        vx, vy, density=1.0, plot_args={"color": "gray", "linewidth": 0.6}
                    )
                elif ptype == 1:
                    ...
                    slc_xy.annotate_streamlines(
                        bx, by, density=1.0, plot_args={"color": "gray", "linewidth": 0.6}
                    )
            ...
            try:
                if ptype == 0:
                    ...
                    slc_xz.annotate_streamlines(
                        vx, vz, density=1.0, plot_args={"color": "gray", "linewidth": 0.6}
                    )
                elif ptype == 1:
                    ...
                    slc_xz.annotate_streamlines(
                        bx, bz, density=1.0, plot_args={"color": "gray", "linewidth": 0.6}
                    )
            except BaseException:
                pass
        ...
        outname = (
            outdir
            / Path(
                f_xy.replace("slice_x3", "combined_3x2").replace(".athdf", ".png")
            ).name
        )
