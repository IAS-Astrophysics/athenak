...
def process_frame(f_xy):
    ...
    try:
        ...
        for i, (field, cmap, (zmin, zmax), is_log, ptype) in enumerate(cols_config):
            ...
            # Helper for contours (FIXED: 'levels' -> 'ncont')
            contour_kwargs = {"ncont": 5, "plot_args": {"colors": "white", "linewidths": 0.5, "alpha": 0.7}}
            if zmin is not None: contour_kwargs["clim"] = (zmin, zmax)

            # ------------------------------------
            # TOP ROW: XY Plane
            # ------------------------------------
            slc_xy = yt.SlicePlot(ds_xy, "z", field, center=[0.0, 0.0, 0.0], width=(200.0, "code_length"))
            ...
            try:
                if ptype == 0:
                    ...
                    slc_xy.annotate_streamlines(vx, vy, density=1.0, plot_args={'color': 'gray', 'linewidth': 0.6})
                elif ptype == 1:
                    slc_xy.annotate_contour(field, **contour_kwargs)
                elif ptype == 2:
                    # Magnetic field lines: bx, by
                    slc_xy.annotate_streamlines(bx, by, density=1.0, plot_args={'color': 'gray', 'linewidth': 0.6})
            except: pass
            ...
            # BOTTOM ROW: XZ Plane
            # ------------------------------------
            # Modern yt 4.x convention: explicit orientation
            slc_xz = yt.SlicePlot(
                ds_xz,
                [0, -1, 0],
                field,
                center=[0.0, 0.0, 0.0],
                width=(200.0, "code_length"),
                north_vector=[0, 0, 1],
            )
            ...
            try:
                if ptype == 0:
                    # Fluid flow lines: vx, vz
                    slc_xz.annotate_streamlines(vx, vz, density=1.0, plot_args={'color': 'gray', 'linewidth': 0.6})
                elif ptype == 1:
                    slc_xz.annotate_contour(field, **contour_kwargs)
                elif ptype == 2:
                    # Magnetic field lines: bx, bz
                    slc_xz.annotate_streamlines(bx, bz, density=1.0, plot_args={'color': 'gray', 'linewidth': 0.6})
            except: pass
        ...
        # Save output
        outdir = Path("../plot_check/central_slices")
        outdir.mkdir(parents=True, exist_ok=True)
        outname = outdir / Path(f_xy.replace("slice_x3", "combined_3x2").replace(".athdf", ".png")).name
