...
If the finest active mesh spacing is larger than the smallest tabulated
horizon radius, the puncture excision mask is under-resolved.  With the
\texttt{problem/unresolved\_sink} flag and the replacement flag
\texttt{problem/sink\_replace\_underresolved\_excision}, the problem generator
disables geometric puncture excision at startup and instead applies a
conservative sink over a larger radius.  This path is intended for cheap early
inspiral evolutions where the binary is deliberately unresolved before a later
...
\end{equation}
with the same cubic smoothstep \(S\) used by smooth excision.  Over a step
\(\Delta t\), the damping fraction is
\begin{equation}
  f_{\rm sink}
  =
...
run, the recommended usage is to keep the sink enabled but set
\texttt{sink\_replace\_underresolved\_excision=false}, allowing AMR to add the
inner level and the geometric excision path to operate once the active mesh
resolves the horizons.
