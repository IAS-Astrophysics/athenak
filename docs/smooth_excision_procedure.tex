...
hole's horizon diameter, the puncture excision mask is under-resolved.  With
the \texttt{problem/unresolved\_sink} flag and the replacement flag
\texttt{problem/sink\_replace\_underresolved\_excision}, the problem generator
suppresses only the under-resolved puncture masks and applies a conservative
sink over a larger radius.  This path is intended for cheap early inspiral
evolutions where one or both holes are deliberately unresolved before a later
AMR restart resolves the horizons.
...
\end{equation}
When this condition is satisfied, the sink for that hole is disabled and the
normal puncture radius is restored for that hole.  If only one hole is
resolved, only that hole's sink is disabled; the other hole keeps its sink and
continues to have its puncture mask suppressed.
...
This fallback is not a replacement for resolved horizon excision.  It is a
coarse-grid drain that removes matter and momentum smoothly while preserving
the constrained-transport representation of \(B_f\).  On a restarted zoom
run, the same per-hole logic continues to apply after AMR regridding: the
sink radius follows the local resolution and each sink turns off only after
that hole satisfies the configured cells-across-horizon criterion.
