...
\begin{enumerate}
  ...
  \item load free-data coefficients,
  \item restrict coefficients,
...
\code{SmoothImpl<2/3/4>()} based on \code{z4c/opt.fd\_stencil}.  The smoothing
kernel is red-black colored:
\[
  c = (\mathrm{color}+i+j+k)\bmod 2.
\]
Cells of one color are updated first, boundary/ghost zones are refreshed, then
the other color is updated.
...
\((\Psi,\beta^x,\beta^y,\beta^z)\), finite-differences a local \(4\times4\)
Jacobian of the CTS operator, solves the coupled Newton correction, and applies
the damped update \(u \leftarrow u + \omega\delta u\).  If the local linear
solve is singular, the code falls back to the diagonal update for that point.
...
while preserving parallelism within each color.
...
\begin{itemize}
  ...
  \item non-periodic mesh faces become \code{mg\_zerofixed}.
\end{itemize}
...
is used by gravity, but CTS currently does not expose an \code{id\_solve/mg\_bc}
override.
...
\begin{itemize}
  ...
  \item \textbf{Stencil/halo mismatch}: high-order operators use invalid ghost
        data or silently reduce order.  Check stencil radius and ghost count.
...
local coupling among \(\Psi\) and \(\beta^i\), but still needs damping and can be
