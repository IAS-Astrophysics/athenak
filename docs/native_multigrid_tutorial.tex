...
\begin{enumerate}
  ...
  \item MPI-gather coarsest MeshBlock coefficients and transfer them to root and
        octets.
\end{enumerate}
...
operations see valid ghost data.  Each \code{RecvBoundary()} task depends on
both the matching receive-post task and the matching send/pack task.  This is
important for same-rank neighbors because \code{PackAndSendMG()} fills their
receive buffers directly, and for MPI neighbors because receives must be posted
before the unpack task polls them.
...
coefficient arrays use the same staged fine/coarse exchange and
\code{FillFineCoarseMGGhosts()} completion step as the solution path, so CTS
residuals near fine/coarse interfaces do not read stale free-data ghosts.  They
do not use correction-variable odd reflection from \code{mg\_zerofixed}.
...
\code{FillFineCoarseMGGhosts()}.  MeshBlock fine/coarse exchange is
MPI-complete: \code{PackAndSendMG()}, \code{InitRecvMG()}, and
\code{RecvAndUnpackMG()} include level-mismatched neighbors.  Same-level
neighbors unpack directly into the live array.  Cross-level payloads are staged
in MG boundary buffers and \code{FillFineCoarseMGGhosts()} computes the final
receiver-side ghost values, preserving the MG formulas that combine neighbor
data with local interior values.  CTS coefficients use the same communication
path, preserving the existing MG coefficient halo behavior.

For root/octet paths, octet boundary helpers copy same-level data or
interpolate from coarser neighbors.  CTS also transfers coefficient/free-data
arrays through this hierarchy after an \code{MPI\_Allgatherv} of coarsest
MeshBlock coefficient values, so replicated root/octet coefficient storage is
