...
coefficient/free-data arrays through this hierarchy after an
\code{MPI\_Allgatherv} of coarsest MeshBlock coefficient values, so replicated
root/octet coefficient storage is complete on every rank.

The MeshBlock fine/coarse ghost helper is currently same-rank only.  If a
MeshBlock has a fine/coarse MG neighbor on another MPI rank, the code now aborts
with a clear fatal error instead of silently leaving those ghosts stale or
crashing later in communication.  Until cross-rank fine/coarse MG buffers are
implemented, SMR MPI tests should use decompositions that keep fine/coarse MG
neighbors on the same rank.
