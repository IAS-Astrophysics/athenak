...
\begin{itemize}
  \item MeshBlock coefficient arrays use same-level exchange, physical free-data
        boundary fills, and \code{FillFineCoarseMGGhosts()} in the same order as
        the solution path.
...
The MeshBlock fine/coarse ghost fill remains same-rank only.  A cross-rank
fine/coarse MeshBlock neighbor triggers a fatal diagnostic rather than allowing
CTS residuals or smoothers to read stale coefficient/free-data ghosts.
