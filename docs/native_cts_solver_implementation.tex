...
coarse coefficient interpolation.  The octet bridge defaults to the same
\code{mg\_coarse\_fd\_stencil} policy unless
\code{id\_solve/octet\_fd\_stencil} is explicitly set, and remains guarded for
sixth-order SMR.
...
\begin{enumerate}
  ...
  \item \code{LoadCoefficients(owner\_->u\_free, ...)} loads free-data
        ...
        same-level, physical, and fine/coarse halos on the finest MeshBlock MG
        level.
  \item \code{RestrictCoefficients()} restricts free-data arrays down the block
        ...
        MeshBlock coefficients use the same fine/coarse ghost fill as
        correction variables; root coefficients are filled with periodic copies
        or zero-gradient physical copies, not with correction-variable
        \code{mg\_zerofixed} sign flips.
  \item \code{SetupMultigrid()} initializes levels, tasks, root grids, and octets.
  \item \code{TransferCoefficientsFromBlocksToRoot()} gathers each rank's
        coarsest MeshBlock coefficients with \code{MPI\_Allgatherv}, then
        populates the replicated root and octet coefficient storage.
...
\begin{itemize}
  \item MeshBlock coefficient arrays use same-level exchange, staged
        fine/coarse exchange, physical free-data boundary fills, and
        \code{FillFineCoarseMGGhosts()} in the same order as the solution path.
  \item \code{TransferCoefficientsFromBlocksToRoot()} first makes coarsest block
        coefficients MPI-complete with \code{MPI\_Allgatherv}, then copies them
        to root cells or refined octets on every rank.
...
MeshBlock fine/coarse ghost fills are MPI-complete.  Cross-level MPI payloads
are treated as staging data: same-level neighbors unpack directly into the live
array, while fine/coarse neighbors unpack into boundary buffers and the receiver
then applies the MG fine/coarse formulas that combine staged neighbor data with
local interior values.  The same staged path is used for correction variables
and CTS coefficients.
